...
Public.billsOper_goods = function (val, opt, row) {
	...
	if(system.userName=="admin"){
		if(getQueryVariable("transType")!="150501"){
...
Business.customerCombo = function($_obj, opts){
	...
	var opts = $.extend(true, {
		...
		defaultSelected: 0,
		editable: true,
		extraListHtml: '<a href="javascript:void(0);" id="quickAddCustomer" class="quick-add-link"><i class="ui-icon-add"></i>新增项目</a>',
		maxListWidth: 500,
		cache: false,
...
Business.supplierCombo = function($_obj, opts){
	...
	var opts = $.extend(true, {
		...
		defaultSelected: 0,
		editable: true,
		extraListHtml: '<a href="javascript:void(0);" id="quickAddVendor" class="quick-add-link"><i class="ui-icon-add"></i>新增项目</a>',
		maxListWidth: 500,
		cache: false,
		forceSelection: false,
		maxFilter: 10,
...
//绑定对应数据，传值
Business.djtypeCombo = function($_obj, opts){
	...
	var opts = $.extend(true, {
			//data: parent.SYSTEM.storageInfo/*'/basedata/invlocation.do?action=list&isEnable=1'*/,
			data: function(){
				return (defaultPage.SYSTEM || opts.userData.system).djtypeInfo;
			},
/*			ajaxOptions: {
				formatData: function(data){
					return data.data.items;
...
/*					if(!storageInfo) {
					...
					if(data) {
						data.id = data.id || data.djtypeId;
						parentTr.data('djtypeInfo', {djtypeId: data.id, name: data.typename});
						//storageInfo.id = data.id;
						//storageInfo.name = data.name;
					}
				}
	...
	var djtypeCombo = $_obj.combo(opts).getCombo();
	return djtypeCombo;
};

//绑定对应数据，传值
Business.dingdanCombo = function($_obj, opts){
	...
	var opts = $.extend(true, {
			//data: parent.SYSTEM.storageInfo/*'/basedata/invlocation.do?action=list&isEnable=1'*/,
			data: function(){
				return (defaultPage.SYSTEM || opts.userData.system).dingdanInfo;
			},
/*			ajaxOptions: {
				formatData: function(data){
					return data.data.items;
...
/*					if(!storageInfo) {
					...
					if(data) {
						data.id = data.id || data.dingdanId;
						parentTr.data('djtypeInfo', {dingdanId: data.id, name: data.typename});
						//storageInfo.id = data.id;
						//storageInfo.name = data.name;
					}
				}
	...
	var dingdanCombo = $_obj.combo(opts).getCombo();
	return dingdanCombo;
};
...
Business.billsEvent = function(obj, type, flag){
	...
	if(type !== 'assemble') {
		...
		$('.grid-wrap').on('click', '.goods .ui-icon-ellipsis', function(e){
			//var t=$(this).parent().parent().siblings(".dingdanhao").html();
			var skuMult = false;
		...
		//批量添加
		$('.grid-wrap').on('click', '.material .ui-icon-ellipsis', function(e){
			//var t=$(this).parent().parent().siblings(".dingdanhao").html();
			var skuMult = false;
			if(!$(this).hasClass('disableSku')){
				skuMult = $(this).data('skuMult') || defaultPage.SYSTEM.enableAssistingProp;
			...
			$.dialog({
				...
				height: 510,
				title: '选择提货物资',
				content: 'url:../settings/material_batch',
				data: {
					skuMult: skuMult,
					skey:_self.skey,
					callback: function(newId, curID, curRow){
		...
		//批量添加
		$('.grid-wrap').on('click', '.ordergoods .ui-icon-ellipsis', function(e){
			var ordert=$(this).parent().parent().siblings(".dingdanhao").text();
			var skuMult = false;
			if(!$(this).hasClass('disableSku')){
				skuMult = $(this).data('skuMult') || defaultPage.SYSTEM.enableAssistingProp;
			...
			$.dialog({
					...
					height: 510,
					title: '选择商品',
					content: 'url:../settings/goods_dingdanbatch?dingdanhao='+ordert,
					data: {
						skuMult: skuMult,
						skey:_self.skey,
						callback: function(newId, curID, curRow){
		...
		//批量添加
		$('.grid-wrap').on('click', '.dingdanhao .ui-icon-ellipsis', function(e){			
			var skuMult = false;
			if(!$(this).hasClass('disableSku')){
				skuMult = $(this).data('skuMult') || defaultPage.SYSTEM.enableAssistingProp;
			...
			$.dialog({
					...
					height: 510,
					title: '选择商品',
					content: 'url:../settings/goods_dingdan',
					data: {
						skuMult: skuMult,
						skey:_self.skey,
						callback: function(newId, curID, curRow){
...
Public.gotoDetailItem = function(params){
	...
	var urls = {
		...
		//PUR
		'150501' : {tabid : 'purchase-purchase',text : '物资出库单',right:'PU_QUERY',url : '../purchase/purchase.jsp?id='},
		//PUR
