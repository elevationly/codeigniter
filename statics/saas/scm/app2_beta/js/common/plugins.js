!function(a) {
    ...
    a.Combo.prototype = {
        ...
        _bindEvent: function() {
			...
			//var names=b.input.attr("class");
			//alert(names);
			//if(names=="textbox goodsAuto"){
        ...
        _setSelected: function(a, b) {
            ...
            if (!isNaN(a)) {
                ...
                if (( - 1 > a || a >= d) && (a = -1), this._selectedIndex != a) {
                    ...
                    g = -1 == a ? "": e.text;
                    this.list.find("." + c.listItemCls); (!b || "function" != typeof this.beforeChange || this.beforeChange(f)) && (c.editable && -1 == a && this.focus || this.input.val(g), this._selectedIndex = a, b && "function" == typeof this.onChange && this.onChange(f), this.originSelect && (this.originSelect[0].selectedIndex = a))
                }
            }
        },
        ...
        _filter: function(b) {
            ...
            if (this.opts.cache || ("local" == this.mode && a.isFunction(this.dataOpt) && (this.rawData = this.dataOpt()), this._formatData()), a.isArray(this.formattedData)) {
                ...
                h = [],
                g = {},
                a.isFunction(this.incrementalSearch) && 100 === e.formattedLen && e.filterData.length < d.maxFilter ? e.addQuery === !0 && this.incrementalSearch(f, c) : (this.lastQuery = b, this.list.empty(), this._populateList(this.filterData), this.expand())
            }
        },
...
function(a, b) {
    ...
    D = function(b) {
        ...
        i.field && (i.bound ? e.body.appendChild(h.el) : i.field.parentNode.insertBefore(h.el, i.field.nextSibling), g(i.field, "change", h._onInputChange), i.defaultDate || (c && i.field.value ? i.defaultDate = moment(i.field.value, i.format).toDate() : i.defaultDate = p(i.field.value), i.setDefaultDate = !0));
        var j = i.defaultDate;
...
function($) {
    ...
    _initNode = function(a, b, c, d, e, f, g) {
        if (c) {
            ...
            c[i] && c[i].length > 0 ? (c.isParent = !0, c.zAsync = !0) : (c.isParent = "string" == typeof c.isParent ? tools.eqs(c.isParent, "true") : !!c.isParent, c.open = c.isParent && !a.async.enable ? c.open: !1, c.zAsync = !c.isParent),
            c.isFirstNode = e,
            c.isLastNode = f,
    ...
    data = {
        ...
        getAfterA: function(a, b, c) {
            for (var d = 0,
            e = _init.afterA.length; e > d; d++) _init.afterA[d].apply(this, arguments)
        },
        getBeforeA: function(a, b, c) {
            for (var d = 0,
            e = _init.beforeA.length; e > d; d++) _init.beforeA[d].apply(this, arguments)
        },
        getInnerAfterA: function(a, b, c) {
            for (var d = 0,
            e = _init.innerAfterA.length; e > d; d++) _init.innerAfterA[d].apply(this, arguments)
        },
        getInnerBeforeA: function(a, b, c) {
            for (var d = 0,
            e = _init.innerBeforeA.length; e > d; d++) _init.innerBeforeA[d].apply(this, arguments)
        },
        getCache: function(a) {
        ...
        initCache: function(a) {
            for (var b = 0,
            c = _init.caches.length; c > b; b++) _init.caches[b].apply(this, arguments)
        },
        initNode: function(a, b, c, d, e, f) {
            for (var g = 0,
            h = _init.nodes.length; h > g; g++) _init.nodes[g].apply(this, arguments)
        },
        initRoot: function(a) {
            for (var b = 0,
            c = _init.roots.length; c > b; b++) _init.roots[b].apply(this, arguments)
        },
        ...
        setZTreeTools: function(a, b) {
            for (var c = 0,
            d = _init.zTreeTools.length; d > c; c++) _init.zTreeTools[c].apply(this, arguments)
        },
    ...
    event = {
        bindEvent: function(a) {
            for (var b = 0,
            c = _init.bind.length; c > b; b++) _init.bind[b].apply(this, arguments)
        },
        unbindEvent: function(a) {
            for (var b = 0,
            c = _init.unbind.length; c > b; b++) _init.unbind[b].apply(this, arguments)
        },
        bindTree: function(a) {
            var b = {
                treeId: a.treeId
    ...
    view = {
        ...
        makeNodeIcoClass: function(a, b) {
            var c = ["ico"];
            return b.isAjaxing || (c[0] = (b.iconSkin ? b.iconSkin + "_": "") + c[0], b.isParent ? c.push(b.open ? consts.folder.OPEN: consts.folder.CLOSE) : c.push(consts.folder.DOCU)),
        ...
        onMousedownNode: function(c, d) {
            function e(c) {
                ...
                if (1 == k.dragFlag) {
                    ...
                    g = o[0],
                    aa && r.isChildOrSelf(c.target, C.treeId) && (!w && (c.target.id == C.treeId || ba || ca || da || ea) && (B || !B && g.parentTId) && (w = C.treeObj), Y ? C.treeObj.scrollTop(C.treeObj.scrollTop() - 10) : Z && C.treeObj.scrollTop(C.treeObj.scrollTop() + 10), $ ? C.treeObj.scrollLeft(C.treeObj.scrollLeft() - 10) : _ && C.treeObj.scrollLeft(C.treeObj.scrollLeft() + 10), w && w != C.treeObj && w.offset().left < C.treeObj.offset().left && C.treeObj.scrollLeft(C.treeObj.scrollLeft() + w.offset().left - C.treeObj.offset().left)),
                    p.css({
                    ...
                    if (w && w.attr("id") != C.treeId) {
                        ...
                        if (oa || pa || qa) {
                            ...
                            if ((1 == va || va >= xa && xa >= -.2) && oa ? (ha = 1 - q.width(), ia = ta - q.height() / 2, H = s.move.TYPE_PREV) : (0 == wa || xa >= wa && 1.2 >= xa) && pa ? (ha = 1 - q.width(), ia = null == sa || ja.isParent && ja.open ? ta + ra.height() - q.height() / 2 : sa.offset().top - q.height() / 2, H = s.move.TYPE_NEXT) : (ha = 5 - q.width(), ia = ta, H = s.move.TYPE_INNER), q.css({
                                display: "block",
                                top: ia + "px",
                                left: ua + ha + "px"
                            }), ra.addClass(s.node.TMPTARGET_NODE + "_" + H), (E != G || F != H) && (K = (new Date).getTime()), ja && ja.isParent && H == s.move.TYPE_INNER) {
                                var ya = !0;
                                window.zTreeMoveTimer && window.zTreeMoveTargetNodeTId !== ja.tId ? (clearTimeout(window.zTreeMoveTimer), window.zTreeMoveTargetNodeTId = null) : window.zTreeMoveTimer && window.zTreeMoveTargetNodeTId === ja.tId && (ya = !1),
                                ya && (window.zTreeMoveTimer = setTimeout(function() {
                                    H == s.move.TYPE_INNER && ja && ja.isParent && !ja.open && (new Date).getTime() - K > C.edit.drag.autoOpenTime && r.apply(C.callback.beforeDragOpen, [C.treeId, ja], !0) && (t.switchNode(C, ja), C.edit.drag.autoExpandTrigger && C.treeObj.trigger(s.event.EXPAND, [C.treeId, ja]))
                                },
                                C.edit.drag.autoOpenTime + 50), window.zTreeMoveTargetNodeTId = ja.tId)
                            }
                        } else w = null,
                        G = "",
                        H = s.move.TYPE_INNER,
