...
var THISPAGE = {
	...
	loadGrid: function() {
		...
		$("#grid").jqGrid({
			url: "../scm/invSa/purLists?action=purLists&type=chukudan&timestamp=" + new Date().getTime(),
			...
			gridview: !0,
			multiselect: !0,
			colNames: [/*"操作", */"物料编号","物料描述", "数量","单位","单价", "总金额","单据类型","编号", "项目名"/*, "已" + c + "款"*/, "领料人","是否归档"/*,"备注"*/, "订单来源"],
			colModel: [/*{
				name: "operating",
				width: 60,
				fixed: !0,
		...
		$("#grid").jqGrid({
			url: "../scm/invSa/purLists?action=purLists",
			postData: queryConditions,
			datatype: "json",
			autowidth: !0,
			height: b.h,
	...
	addEvent: function() {
		...
		$("#search").click(function() {
			...
			if(urlParam.Type=="chukudan"){
				...
				queryConditions.matchCon = "请输入单据号或项目名" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.mname = a.$_mname.val(),queryConditions.billNo = a.$_billNo.val(),queryConditions.mnumber = a.$_mnumber.val(),queryConditions.ordernumber = a.$_ordernumber.val(),queryConditions.mdescription = a.$_mdescription.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
