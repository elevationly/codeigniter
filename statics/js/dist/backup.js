...
function init() {
	(12 == parent.SYSTEM.serviceType || 16 == parent.SYSTEM.serviceType) && $("#localRecover").hide(), getBackupData(), $("#start-backup").on("click", function(a) {
		a.preventDefault(), Business.verifyRight("PU_DELETE") && backupConfirm()
	}), $("#localRecover").on("click", function(a) {
		if (12 !== parent.SYSTEM.serviceType || 16 !== parent.SYSTEM.serviceType) {
			if (a.preventDefault(), !Business.verifyRight("PU_DELETE")) return;
			recoverLocalBackup()
		}
...
function getBackupData() {
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "../scm/invSa/queryBackupFile?action=queryBackupFile",
		success: function(a) {
...
function opeFmatter(a, b, c) {
	...
	return '<p data-id="' + c.filename + '" class="operate-wrap operating"><a class="btn-recover ui-icon ui-icon-copy" href="#" title="恢复">恢复</a><a class="btn-del ui-icon ui-icon-trash" href="#" title="删除">删除</a></p>'
}
function backupConfirm() {
	var a = ["<p>为保证备份数据的完整性，<strong></strong>。</p>", "<p>确定执行备份？</p>"];
	$.dialog({
		title: "开始备份",
		id: "backupDialog",
		width: 300,
...
function doBackup() {
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "../scm/invSa/backupshow?action=backupshow",
		success: function(b) {
...
function doDelete(a) {
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "../scm/invSa/deleteBackupFile?action=deleteBackupFile&id=" + a,
		success: function(b) {
			if (200 == b.status) {
...
function recoverBackup(a) {
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "../scm/invSa/recover?action=recover&id=" + a,
		success: function(a) {
