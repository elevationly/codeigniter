...
var THISPAGE = {
	...
	initDom: function() {
		this.$_matchCon = $("#matchCon"),this.$_mname = $("#mname"),this.$_mnumber = $("#mnumber"),this.$_ordernumber = $("#ordernumber"),this.$_mdescription = $("#mdescription"), this.$_beginDate = $("#beginDate").val(system.beginDate), this.$_endDate = $("#endDate").val(system.endDate), this.$_matchCon.placeholder(), this.$_beginDate.datepicker(), this.$_endDate.datepicker()
	},
	loadGrid: function() {
		function a(a, b, c) {
			var d;
		...
		$("#grid").jqGrid({
			url: "../scm/invSa/zuofei?action=zuofei",
			...
			gridview: !0,
			multiselect: !0,
			colNames: [/*"操作", */"物料编号","物料描述", "数量","单位","单价", "总金额","采购订单号","单据类型","编号", "项目名"/*, "已" + c + "款"*/, "领料人"/*,"备注"*/, "订单来源"],
			colModel: [/*{
				name: "operating",
				width: 60,
				fixed: !0,
	...
	addEvent: function() {
		...
		$("#search").click(function() {
			...
			if(urlParam.Type=="chukudan"){
				...
				queryConditions.matchCon = "请输入单据号或项目名" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.mname = a.$_mname.val(),queryConditions.type = "chukudan", queryConditions.mnumber = a.$_mnumber.val(),queryConditions.ordernumber = a.$_ordernumber.val(),queryConditions.mdescription = a.$_mdescription.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
		
			}else{
				queryConditions.matchCon = "请输入单据号或项目名" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.mname = a.$_mname.val(),queryConditions.mnumber = a.$_mnumber.val(),queryConditions.ordernumber = a.$_ordernumber.val(),queryConditions.mdescription = a.$_mdescription.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
