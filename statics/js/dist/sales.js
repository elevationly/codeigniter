var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		init: function(a) {
			"150502" == urlParam.transType ? this.mod_PageConfig = Public.mod_PageConfig.init("purchaseBack") : this.mod_PageConfig = Public.mod_PageConfig.init("purchase"), SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_INAMOUNT || (hiddenAmount = !0, $("#amountArea").hide()), this.loadGrid(a), this.initDom(a), this.initCombo(), a.id > 0 && a.checked==1 ? this.disableEdit() : (this.editable = !0, $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
		...
		if ("add" !== a.status || a.salesId) var c = ["id", a.salesId];
			...
			}), a.description && this.$_note.val(a.description), this.$_discountRate.val(a.disRate), this.$_deduction.val(a.disAmount), this.$_sign.val(a.sign),this.$_handwritingimg.attr("src", "data:image/gif;base64,"+a.sign),this.$_sales.find('input').val(a.liname),this.$_discount.val(a.amount), this.$_payment.val(a.rpAmount), this.$_arrears.val(a.arrears), requiredMoney && ($("#accountWrap").show(), SYSTEM.isAdmin !== !1// || SYSTEM.rights.SettAcct_QUERY ? this.accountCombo = Business.accountCombo($("#account"), {
			/*	width: 112,
				height: 300,
				emptyOptions: !0,
				addOptions: {
			...
			if(SYSTEM.userName=="admin"){
				...
				d = '<a id="add" class="ui-btn ui-btn-sp">新增</a><!--<a id="edit" class="ui-btn">保存</a>-->',
				e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a class="ui-btn ui-btn-sp" id="savaAndAdd">提交审核</a>',
				e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a class="ui-btn ui-btn-sp" id="savaAndAdd">提交审核</a>',
				f = "",
				g = "";
				}else{
					var c = '<!--<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>-->',
				d = '<a id="add" class="ui-btn ui-btn-sp">新增</a><!--<a id="edit" class="ui-btn">保存</a>-->',
				e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invPu/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a><a id="zuofei" class="ui-btn ui-btn-sp">作废</a>',
				//e = '<a href="../scm/invPu/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				f = "",
				g = "";
				}

				if(urlParam.flag=="list"){
					billRequiredCheck ? (f = '<a class="ui-btn ui-btn-sp" id="audit">审核</a><!--<a class="ui-btn ui-btn-sp" id="Reject">驳回</a>--><a id="zuofei" class="ui-btn ui-btn-sp">作废</a>') : this.$_checkName.parent().hide();
					if(a.checked==2){
						$("#mark").addClass("has-noaudit");
					}
					if(a.checked==3){

						$("#mark").addClass("zuofei");

					}

				}else{
					billRequiredCheck ? (f = '<a class="ui-btn ui-btn-sp" id="savaAndAdd">提交审核</a><a class="ui-btn" id="daoru">导入</a>') : this.$_checkName.parent().hide();
				}

			//billRequiredCheck ? (f = '<a class="ui-btn ui-btn-sp" id="savaAndAdd">提交审核</a>' : this.$_checkName.parent().hide();
...
//}
					var c = {
						...
						goodsnumber:a.invNumber,
						number:a.invNumber,
						invNumber:a.invNumber,
						mdescription:a.describe,
						skuName: b.skuName || "",
						mainUnit: a.company,
						name: a.company,
						unitId: b.unitId,
					...
					$("#" + num).data("goodsInfo",c);
					$("#grid").jqGrid("setRowData", num, c);
...
//}
			...
			function k(a, b, c) {
				...
				if ("get" === b) {
					if ("" !== $(".dingdanAuto").getCombo().getValue()) return $(a).val();
					var d = $(a).parents("tr"),
						e = d.data("dingdanInfo") || {};
					return THISPAGE.dingdanCombo.selectByIndex(e.unitId || e.id), e.name || ""
				}
				"set" === b && $("input", a).val(c)
			}
			...
			function dz()
			 {
				 $("#initCombo").append($(".dingdanAuto").val(""))
			 }
			function m(a, b) {
				var c = $(".dateAuto")[0];
				return c
...
//}
			...
			w.mod_PageConfig.gridReg(C, B), B = w.mod_PageConfig.conf.grids[C].colModel, $("#grid").jqGrid({
				...
				//编辑当前单元格使用
					...
					if (THISPAGE.curID = a, "goodsnumber" === b && (f(), $("#" + d + "_goods", "#grid").val(c), THISPAGE.goodsCombo.selectByText(c)), "skuName" === b) {
						f();
		...
		goodsEdittypeInit: function() {
			...
			function c() {
				$("#initCombo").append($(".skuAuto").val("").unbind("focus.once"))
			}
			0 != $("#grid")[0].p.savedRow.length && $("#grid").jqGrid("saveCell", $("#grid")[0].p.savedRow[0].id, $("#grid")[0].p.savedRow[0].ic), THISPAGE.$_barCodeInsert.hasClass("active") ? $("#grid").jqGrid("setColProp", "goodsnumber", {
				edittype: "text",
				editoptions: null
			}) : $("#grid").jqGrid("setColProp", "goodsnumber", {
				edittype: "custom",
				editoptions: {
					custom_element: a,
					custom_value: b,
		...
		reloadData: function(a) {
			...
			$("#grid").jqGrid("setGridParam", {
				...
				"edit" === a.status ? /*this.editable || */(c.enableEdit(), $("#groupBtn").html(c.btn_edit + c.btn_audit),
				$("#mark").removeClass("has-audit")) : /*this.editable && */(c.disableEdit(), $("#groupBtn").html(c.btn_view + c.btn_reaudit), $("#mark").addClass("has-audit"))
		},
		initCombo: function() {
			var a = this;
			...
			this.goodsCombo = Business.billGoodsCombo($(".goodsAuto"), {
				...
				data: []
			}), this.storageCombo = Business.billStorageCombo($(".storageAuto")), this.unitCombo = Business.unitCombo($(".unitAuto"),this.dingdanCombo = Business.dingdanCombo($(".dingdanAuto"), {
				//调对应的方法，给其传值
				defaultSelected: -1,
				forceSelection: !1
		...
		disableEdit: function() {
			this.customerCombo.disable(), this.$_date.attr("disabled", "disabled").addClass("ui-input-dis"),this.$_sales.find('input').attr("disabled", "disabled").addClass("ui-input-dis"),this.$_sales.find('i').removeClass("trigger"), this.$_note.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_discountRate.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_deduction.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_payment.attr("disabled", "disabled").addClass("ui-input-dis"), /*this.accountCombo.disable(),*/ $("#grid").jqGrid("setGridParam", {
				cellEdit: !1
			}), this.editable = !1
		},
		enableEdit: function() {
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				var b = $(this).siblings(),
				 ...
				 //alert($(this).html("asd"));
				//var a="<div class='ui-droplist-wrap' style='position: absolute; top: 69.2px; z-index: 1000; width: 122.4px;height:50px; display: block; left: 0px;border:1px solid red;'></div>";
				...
				if (b.preventDefault(), Business.verifyRight("PU_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invSa/updateInvPu?action=updateInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_CHECK")) {
					...
					d && c.ajaxPost("../scm/invSa/checkInvPu?action=checkInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (originalData.id = b.data.id,$("#mark").removeClass(),$("#mark").removeClass("has-audit"),$("#mark").addClass("has-audit"), a.$_checkName.html(SYSTEM.realName).parent().show(), $("#edit").hide(), a.disableEdit(), $("#groupBtn").html(a.btn_view + a.btn_reaudit), parent.Public.tips({
							content: "审核成功！"
						})) : parent.Public.tips({
							type: 1,
							content: b.msg
			...
			}), $(".wrapper").on("click", "#Reject", function(b) {
				if (b.preventDefault(), Business.verifyRight("PU_CHECK")) {
					var c = $(this),
						d = THISPAGE.getPostData({
							checkSerNum: !0
						});
					d && c.ajaxPost("../scm/invSa/RejectInvPu?action=RejectInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (originalData.id = b.data.id, $("#mark").removeClass(),$("#mark").addClass("has-noaudit"), a.$_checkName.html(SYSTEM.realName).parent().show(), $("#edit").hide(), a.disableEdit(), $("#groupBtn").html(a.btn_view + a.btn_reaudit), parent.Public.tips({
							type: 1,
							content: "审核不成功！"
						})) : parent.Public.tips({
							type: 1,
			...
			}), $(".wrapper").on("click", "#zuofei", function(b) {
				if (b.preventDefault(), Business.verifyRight("SA_zuofei")) {
				$.dialog.confirm("您确定要作废吗？", function() {
					$.dialog.prompt("请再次输入密码！", function(as) {
					...
					d && c.ajaxPost("../scm/invSa/zuofeiCheckInvPu?action=zuofeiCheckInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (originalData.id = b.data.id, $("#mark").removeClass("has-audit"),$("#mark").addClass("zuofei"), a.$_checkName.html(SYSTEM.realName).parent().show(), $("#edit").hide(), a.disableEdit(), $("#groupBtn").html(a.btn_view + a.btn_reaudit), parent.Public.tips({
							type: 1,
							content: "作废成功！"
						})) : parent.Public.tips({
							type: 1,
				...
				if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invSa/rsBatchCheckInvPu?action=rsBatchCheckInvPu", {
						id: originalData.id
					}, function(b) {
				...
				//if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					...
					$.dialog({
						content: "url:../settings/Contractorderss",
						data: {
							title: 'excel上传',
							id: "<?php echo $billNo?>",
							callback: function() {
						...
						lock: !0,
						close: function() {
							c.ajaxPost("../scm/invSa/daorusales?action=daorusales", {
								id: 2
							}, function(b) {
								for(var i=0;i<b.length;i++){
									THISPAGE.loadGrid(b[i],"d",i+1);
								}

							});
							c.ajaxPost("../scm/invPu/daorudelete?action=daorudelete", {
								id: 2
							}, function(b) {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					...
					var pl='http://'+host+'/index.php/scm/invSa?action=editSale&id='+param.id+'&flag=list&transType=150501';
					var path=changeURLArg(pl,'id',a.idList[a.idPostion]);
			...
			}), $("#grid").on("change", '.dingdanhao', function(a) {

				//alert($(this).find("input").val());
				$(this).nextAll().eq(0).text("");
				$(this).nextAll().eq(0).html("");
				$(this).nextAll().eq(0).append("<div class='pr'><span class='ui-icon-ellipsis'></span></div>");
				$(this).nextAll().eq(3).text("");
				$(this).nextAll().eq(4).text("");
				$(this).nextAll().eq(5).text("");
				$(this).nextAll().eq(11).text("");
				$(this).nextAll().eq(12).text("");
				$(this).nextAll().eq(13).text("");
			...
			}), $("#grid").on("blur", '.dingdanhao', function(a) {

				$(".footrow").find(".qtyupd").html("");
			}), $("#grid").on("focus", '.dingdanhao', function(a) {

				$(".footrow").find(".qtyupd").html("");
			}), $("#grid").on("change", '.qtyupd', function(a) {
				//alert($(this).nextAll().eq(2).text());
				var qtys=$(this).children().val();
				var priceas=$(this).next().next().next().next().next().next().next().text();
				//var amonut=parseFloat($(this).next().next().next().next().next().next().next().text())*parseFloat(qtys);
				//$(this).next().next().next().next().next().next().next().next().text(amonut);
				/*$(this).nextAll().eq(0).text("");
				$(this).nextAll().eq(0).html("");
				$(this).nextAll().eq(0).append("<div class='pr'><span class='ui-icon-ellipsis'></span></div>");
				$(this).nextAll().eq(3).text("");
...
//}
					...
					if (i) {
						...
						*/
						...
						f = {
							invId: i.id || i.invId,
							ordernumber:h.ordernumber || i.ordernumber,
							goodsnumber:i.number || h.goodsnumber || i.invNumber,
							//invNumber: i.number || h.goodsnumber || i.invNumber,
							invName: i.name,
							invSpec: i.spec || "",
							skuId: j.id || -1,
							skuName: j.name || "",
							unitId: l.unitId || -1,
							mainUnit: h.mainUnit || l.name || i.name || "",
							qty: h.qty,
							price: h.price,
							...
							//locationId: k.id,
							//locationName: h.locationName || k.name,
							locationName: h.locationName || i.locationName,
		...
		getPostData: function(a) {
			...
			var sales = c.$_sales.find("input");
			if ("" === sales.val()||"(空)" === sales.val()) return c.$_sales.removeData(""), parent.Public.tips({
				type: 2,
				content: "请选择领料人！"
			}), c.salesCombo.active = !0, c.salesCombo.doQuery(), c.salesCombo.input.focus(), !1;
			...
			for(i in f ){
				...
				for(j in f[i]){
					...
					//alert(n);
					if(j=="price" && f[i][j]!="" && n!=0){
						//alert(n);
						sum+=parseInt(f[i][j])*n;
						//alert(f[i][j]);
					}
					if(j=="BillName" && f[i][j]==""){
						return defaultPage.Public.tips({
					type: 2,
					content: "单据类型不能为空！"
				}), !1;
					}
			...
			var sign = c.$_sign.val();
			// if ("" === sign) return c.$_sign.removeData(""), parent.Public.tips({
			// 	type: 2,
			// 	content: "请签名！"
			// }), !1;
			//alert(sum);
			if (!f) return !1;
			if (f.length > 0) {
				//var a=parseInt($("#grid").jqGrid("footerData", "get").amount);
				//var b=parseInt($("#grid").jqGrid("footerData", "get").qty);
				//alert(b);
				var g = $.trim(b.$_note.val()),
					h = {
						...
						buId: e.id,
						contactName: e.name,
						liname:this.$_sales.find("input").val(),
						sign:this.$_sign.val(),
...
$(function() {
	if (urlParam.id) {
		...
		if (!hasLoaded) {
						...
						for (var e = 0; e < b.length; e++) {
							...
							if ("object" == typeof f) {
								...
								if (f) {
									...
									var a = $.extend(!0, {}, f);
									if (a.goodsnumber = i, a.id = j, a.qty = a.qty || 1, d) var k = $("#grid").jqGrid("setRowData", Number(d), {});
									else {
			...
			} else Public.ajaxGet("../scm/invSa/update?action=update", {
				id: urlParam.id
			}, function(b) {
