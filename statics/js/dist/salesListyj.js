...
var THISPAGE = {
	...
	initDom: function() {
		this.$_matchCon = $("#matchCon"),this.$_mname = $("#mname"),this.$_mnumber = $("#mnumber"),this.$_ordernumber = $("#ordernumber"), this.$_beginDate = $("#beginDate").val(system.beginDate), this.$_endDate = $("#endDate").val(system.endDate), this.$_matchCon.placeholder(), this.$_beginDate.datepicker(), this.$_endDate.datepicker()
	},
	loadGrid: function() {
		function a(a, b, c) {
			var d;
		...
		$("#grid").jqGrid({
			url: "../scm/invSa/purListyj?action=purListyj&type=chukudan&timestamp=" + new Date().getTime(),
			...
			gridview: !0,
			multiselect: !0,
			colNames: ["操作", "物料编号","物料描述", "库存数量","单位","预警数量", "备注"/*,"单据类型","编号", "项目名", "已" + c + "款", "领料人","是否归档","备注"*/, "订单来源"],
			colModel: [{
				name: "operating",
				width: 60,
				fixed: !0,
		...
		$("#grid").jqGrid({
			url: "../scm/invSa/purListyj?action=purListyj",
			postData: queryConditions,
			datatype: "json",
			autowidth: !0,
			height: b.h,
	...
	addEvent: function() {
		...
		if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
			...
			Public.ajaxGet("../scm/invSa/showyj?action=showyj", {
				id: b
			}, function(a) {
				$(".yjgoodsnumnerupd").val(a.number);
				$(".yjnumupd").val(a.yjnum);
		...
		$(".wrapper").on("click", ".ui-btn-sp", function(a) {
			if (a.preventDefault(), Business.verifyRight("QTZC_ADD")) {
			$(".yjbox").show();
		$(".Covering").show();
			}
				//alert(b);
				...
				$.dialog.confirm("您确定要删除该记录吗？", function() {
					$.dialog.prompt("请再次输入密码！", function(a) {
						Public.ajaxGet("../scm/invPu/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								Public.ajaxGet("../scm/invSa/deleteyj?action=deleteyj", {
									id: b
								}, function(a) {
		...
		$(".wrapper").on("click", "#guidang", function(a) {
			...
			if (!c) return void parent.Public.tips({
				...
				content: "请先选择需要归档的项！"
						...
						Public.ajaxGet("../scm/invPu/showpwd?action=showpwd", {
							...
							if(200 === a.status){	
								Public.ajaxPost("../scm/invSa/guidang?action=guidang", {
									id: c
								}, function(a) {
			...
			if(urlParam.Type=="chukudan"){
				var f = "../scm/invSa/exportInvPuh?action=exportInvPuh" + d;
			}else{
		...
		$("#search").click(function() {
			...
			if(urlParam.Type=="chukudan"){
				...
				queryConditions.matchCon = "请输入单据号或项目名" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.mname = a.$_mname.val(),queryConditions.type = "chukudan", queryConditions.mnumber = a.$_mnumber.val(),queryConditions.ordernumber = a.$_ordernumber.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
		
			}else{
				queryConditions.matchCon = "请输入单据号或项目名" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.mname = a.$_mname.val(),queryConditions.mnumber = a.$_mnumber.val(),queryConditions.ordernumber = a.$_ordernumber.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
