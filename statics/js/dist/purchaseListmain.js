...
var THISPAGE = {
	...
	loadGrid: function() {
		...
		$("#gridss").jqGrid({
			url: "../scm/invPu/purListmain?action=purListmain",
			...
			loadComplete: function(a) {
				...
				"150502" == queryConditions.transType && $("#gridss").find(".jqgrow").addClass("red")
			},
			loadError: function() {},
	...
	addEvent: function() {
		...
		if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
			...
			var b = $(this).parent().data("id"),
				c = $("#gridss").jqGrid("getRowData", b),
				d = 1 == c.disEditable ? "&disEditable=true" : "",
				e = ($("#gridss").jqGrid("getDataIDs"), "物资出库单"),
				f = "purchase-purchase";
			"150502" == queryConditions.transType ? (e = "购货退货单", f = "purchase-purchaseBack", parent.cacheList.purchaseBackId = $("#gridss").jqGrid("getDataIDs")) : parent.cacheList.purchaseId = $("#gridss").jqGrid("getDataIDs"), parent.tab.addTabItem({
				tabid: f,
				text: e,
						...
						Public.ajaxGet("../scm/invPu/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								Public.ajaxGet("../scm/invPu/delete?action=delete", {
									...
									200 === a.status ? ($("#gridss").jqGrid("delRowData", b), parent.Public.tips({
										content: "删除成功！"
									})) : parent.Public.tips({
										type: 1,
										content: a.msg
		...
		$(".wrapper").on("click", "#btn-batchDel", function(a) {
			...
			var b = $("#gridss").jqGrid("getGridParam", "selarrrow"),
				c = b.join();
			...
			$(".wrapper").on("click", "#audit", function(a) {
				...
				return c ? void Public.ajaxPost("../scm/invPu/batchCheckInvPu?action=batchCheckInvPu", {
					...
					if (200 === a.status) {
						for (var c = 0, d = b.length; d > c; c++) $("#gridss").setCell(b[c], "checkName", system.realName), $("#" + b[c]).removeClass("gray");
						parent.Public.tips({
							content: "审核成功！"
						})
					} else parent.Public.tips({
				...
				return c ? void Public.ajaxPost("../scm/invPu/rsBatchCheckInvPu?action=rsBatchCheckInvPu", {
					...
					if (200 === a.status) {
						for (var c = 0, d = b.length; d > c; c++) $("#gridss").setCell(b[c], "checkName", "&#160;"), $("#" + b[c]).addClass("gray");
						parent.Public.tips({
							content: "反审核成功！"
						})
					} else parent.Public.tips({
