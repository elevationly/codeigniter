...
//}			
			...
			function dz() {
				$("#initCombo").append($(".djtypeAuto").val("利库单"))
			 }
			function m(a, b) {
				var c = $(".dateAuto")[0];
				return c
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_CHECK")) {
					...
					d && c.ajaxPost("../scm/invPu/RejectInvPu?action=RejectInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				$.dialog.confirm("您确定要作废吗？", function() {
					...
					d && c.ajaxPost("../scm/invPu/zuofeiCheckInvPu?action=zuofeiCheckInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				//if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					...
					$.dialog({
						...
						close: function() {
							c.ajaxPost("../scm/invPu/daoru?action=daoru", {
								id: 2
							}, function(b) {				
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					...
					var pl='http://'+host+'/index.php/scm/invPu?action=editPur&id='+param.id+'&flag=list&transType=150501';
