function callbackSp() {
	...
	if (e > 0) {
		...
		if (i.id = i.id.split("_")[0], h.SYSTEM.goodsInfo.push(i), "" === i.spec) var j = i.number;
...
function callback() {
	...
	if (isSingle) {
		...
		if (i.id = i.id.split("_")[0], delete i.amount, defaultPage.SYSTEM.goodsInfo.push(i), "" === i.spec) var j = i.number;
	    //else var j = i.number + " " + i.name + "_" + i.spec;
		else var j = i.number;
		...
		for (rowid in addList) {
			...
			var n = $.extend(!0, {}, i);
			if (n.goodsnumber = j, n.id = k, n.qty = n.qty || 1, b) var o = parent.$("#grid").jqGrid("setRowData", Number(b), {});
			else {
...
var api = frameElement.api,
	...
	THISPAGE = {
		...
		loadGrid: function() {
			...
			$("#grid").jqGrid({
				url: "../basedata/lading?action=list",
