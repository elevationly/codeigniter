function init() {
	SYSTEM.ISSERNUM || SYSTEM.enableAssistingProp || SYSTEM.ISWARRANTY || $("#tab").find(".tabItem:eq(2)").hide(), void 0 !== cRowId ? Public.ajaxPost("../basedata/lading/query?action=query", {
		id: cRowId
	}, function(a) {
...
function postCustomerData() {
	if ("add" == oper) {
		cancleGridEdit();
		var a = $("#name").val();
		Public.ajaxPost("../basedata/lading/checkName?action=checkName", {
			name: a
		}, function(b) {
...
function postData() {
	...
	if (b) {
		...
		}), Public.ajaxPost("../basedata/lading/" + ("add" == oper ? "add" : "update"), b, function(d) {
			if (200 == d.status) {
...
function getCustomerData() {
	...
	if (a) {
		var b = {
			...
			name: $.trim($("#name").val()),
			spec: $.trim($("#spec").val()),
			jianxing: $('#jianxing').val(),
			unitName: $.trim($("#unitname").val()),
...
function initField() {
							//for(i in rowData ){
  //alert(i);           //获得属性 
  //alert(rowData[i]);  //获得属性值
//}	
	$("#note").placeholder(),
		"edit" == oper ?
			($("#number").val(rowData.number), $("#name").val(rowData.name),
				$("#unitCost").val(rowData.unitCost),
				$("#goods").val(rowData.goods),
				$("#spec").val(rowData.spec),
				$("#remark").val(rowData.remark),
				$("#unitname").val(rowData.unitName),
				$("#jianxing").val(rowData.jianxing),
			...
			void 0 != rowData.salePrice && ($("#salePrice").val(Public.numToCurrency(rowData.salePrice, pricePlaces)),
				...
				$("#minInventory").val(rowData.lowQty), $("#maxInventory").val(rowData.highQty),
			rowData.remark && $("#note").val(rowData.remark), $("#barCode").val(rowData.barCode),
				$("#length").val(rowData.length), $("#width").val(rowData.width),
				$("#height").val(rowData.height), $("#weight").val(rowData.weight),
	...
	api.opener.parent.SYSTEM.isAdmin || (rights.AMOUNT_INAMOUNT || $("#purchasePrice").closest("li").hide(),
	rights.AMOUNT_OUTAMOUNT || ($("#salePrice").closest("li").hide(), $("#wholesalePrice").closest("li").hide(),
		$("#vipPrice").closest("li").hide(), $("#discountRate1").closest("li").hide(),
		$("#discountRate2").closest("li").hide())), SYSTEM.enableStorage && $("#jdInfo").show(),
	SYSTEM.enableAssistingProp && ($(".prop-wrap").show().on("click", "input", function(a) {
		var b = $(this);
...
function initEvent() {
	...
	categoryTree = Public.categoryTree($category, b), $("#specs").blur(function() {
		...
		"" == a || "edit" == oper && a == rowData.spec || Public.ajaxPost("../basedata/lading/checkSpec?action=checkSpec", {
			spec: a
		}, function(b) {
		...
		if (c && c.skuId && "edit" == oper) $.dialog.confirm("删除的分录将不能恢复，请确认是否删除？", function() {
			Public.ajaxPost("../basedata/lading/deleteSku?action=deleteSku", {
				invId: rowData.id,
				skuId: c.skuId
			}, function(a) {
				if (200 === a.status) {
	...
	$form.validator({
		rules: {
			...
			checkCode: function(a, b) {
				...
				return $.ajax({
					type: "POST",
					url: "../basedata/lading/checkBarCode?action=checkBarCode",
					data: {
						barCode: c
					},
					dataType: "json",
			...
			myRemote: function(a, b, c) {
				return c.old.value === a.value || $(a).data("tip") === !1 && a.value.length > 1 ? !0 : $.ajax({
					url: "../basedata/lading/getNextNo?action=getNextNo",
					type: "post",
					data: "skey=" + a.value,
					dataType: "json",
					success: function(b) {
