...
function getStores() {
	SYSTEM.isAdmin || SYSTEM.rights.CLOUDSTORE_QUERY ? Public.ajaxGet("", {}, function(a) {
...
function getLogistics() {
	SYSTEM.isAdmin || SYSTEM.rights.EXPRESS_QUERY ? Public.ajaxGet("", {}, function(a) {
...
var dataReflush, list = {
	...
	purchases: {
		name: "作废出库单",
		href: "../scm/invPu?action=initPurListzuofei",
		dataRight: "PU_ADD",
		target: "purchase",
	},
	 purchaseBack: {
		name: "出库单汇总",
		href: "../scm/invPu?action=initPurList&Type=chukudan",
		dataRight: "PU_ADD",
		target: "purchase",
	},
	lading: {
		name: "物资提货单",
		href: "../scm/lading?action=initPur",
		dataRight: "LADING_ADD",
		target: "lading",
		list: "../scm/lading?action=initPurList"
	},
	errorLading: {
		name: "作废提货单",
		href: "../scm/lading?action=initPurListzuofei",
		dataRight: "LADING_ADD",
		target: "lading"
	},
	cancelLading: {
		name: "作废单汇总",
		href: "../scm/lading?action=initPurList&Type=chukudan&checkStatus=3",
		dataRight: "LADING_ADD",
		target: "lading"
	},
	AllLading: {
		name: "提货单汇总",
		href: "../scm/lading?action=initPurList&Type=chukudan",
		dataRight: "LADING_ADD",
		target: "lading"
	},
	...
	*/
	...
	sales: {
		...
		dataRight: "SA_ADD",
		target: "sales",
		list: "../scm/invSa?action=initSaleListSearch&transType=150602"
	},
	saless: {
		name: "作废领料单",
		href: "../scm/invSa?action=initSaleListzuofei&transType=150602",
		dataRight: "SA_ADD",
		target: "sales",
	},
	salesCancel: {
		name: "作废单汇总",
		href: "../scm/invSa?action=initSaleListCancel&Type=chukudan",
		dataRight: "SA_ADD",
		target: "sales",
	},
	salesBack: {
		name: "领料单汇总",
		href: "../scm/invSa?action=initSaleList&Type=chukudan",
		dataRight: "SA_ADD",
		target: "sales",
	},
	...
	ladingStock: {
		name: "提货单物资库",
		href: "../settings/lading_list",
		dataRight: "INVENTORY_QUERY",
		target: "storage",
	},
	otherWarehouses: {
		name: "领料单物资库",
		href: "../settings/orders_list",
		dataRight: "INVENTORY_QUERY",
		target: "storage",
	...
	otherOutbound: {
		name: "出库汇总",
		href: "../scm/invSa?action=initSaleLists&Type=chukudan",
		dataRight: "OO_QUERY",
		target: "storage",
	...
	inventory: {
		name: "库存预警",
		href: "../scm/invSa?action=initSaleListyj&Type=chukudan",
		dataRight: "QTZC_QUERY",
		target: "storage"
	},
	cangku: {
		name: "仓储物资库",
		href: "../settings/cangku_list",
		dataRight: "QTSR_ADD",
		target: "cangku"
	},
	cangkus: {
		name: "仓储出库汇总",
		href: "../settings/cangku_listhuizong",
		dataRight: "QTSR_UPDATE",
		target: "cangku"
	},
	/*otherOutbound: {
		name: "商品类别",
			...
			if (this.sublist && this.obj) {
				...
				for (var c in a) if (!a[c].disable) {
					var d = a[c],
						...
						f = d.id ? "id=" + d.id : "",
						g = d.id ? "" : "rel=pageTab",
						topAttr = (d.href && d.href.indexOf("login/out") !== -1) ? ' target="_top"' : "",
						h = "";
					if (d.list) {
						var i = d.name + "记录";
						h = "<i " + f + ' tabTxt="' + i + '" tabid="' + d.target.split("-")[0] + "-" + c + 'List" ' + g + ' href="' + d.list + '" data-right="' + d.dataRight.split("_")[0] + '_QUERY">查询</i>'
					}
					var j = "<li><a " + f + topAttr + ' tabTxt="' + d.name + '" tabid="' + d.target.split("-")[0] + "-" + c + '" ' + g + ' href="' + d.href + '" data-right="' + d.dataRight + '">' + d.name + h + "</a></li>";
