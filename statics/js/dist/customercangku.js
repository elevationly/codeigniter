...
function initValidator() {
	$_form.validator({
		...
		valid: function(a) {
			var mdescription = $.trim($("#mdescription").val());
			var goodsnumber = $.trim($("#goodsnumber").val());
			var inventoryNew = $.trim($("#inventoryNew").val());
			var inventoryOld = $.trim($("#inventoryOld").val());
			...
			var flagNo = $.trim($("#flagNo").val());
			var flagtime = $.trim($("#flagtime").val());
			var flagcontact = $.trim($("#flagcontact").val());
			var beizhu = $.trim($("#beizhu").val());
			var daohuo = $.trim($("#daohuo").val());
			Public.ajaxPost("../basedata/inventory/addcangku?action=addcangku", {
				...
				/*return 200 != a.status ? !0 : void parent.parent.Public.tips({
							type: 3,
							content: "新增仓库成功！"
