var curRow, curCol, loading = null,
	...
	THISPAGE = {
		...
		initDom: function() {
			this.$_storage = $("#storage"), this.$_category = $("#category"), this.$_goods = $("#goods"), this.$_ordernumber = $("#ordernumber"), this.$_note = $("#note"), this.chkField = $("#chkField").cssCheckbox(), this.storageCombo = $("#storage").combo({
				data: function() {
		...
		loadGrid: function(a) {
			...
			$("#grid").jqGrid({
				...
				formatter: function(a, b, c) {
					var d = '<div class="operating" data-id="' + c.id + '"><span class="ui-icon ui-icon-pencil" title="修改" id="btn-upd"></span><span class="ui-icon ui-icon-trash" title="删除"></span></div>';
					return d
				},
				title: !1
			},{
		...
		addEvent: function() {
			...
			$("#search").click(function() {
				queryConditions = {
					...
					goods: a.$_goods.val(),
					ordernumber: a.$_ordernumber.val(),
					showZero: 0,
					isSerNum: 0
				};
				...
				return Business.verifyRight("PD_EXPORT") ? void $(this).attr("href", "../scm/invOi/exportToPD?action=exportToPD&locationId=" + queryConditions.locationId + "&categoryId=" + queryConditions.categoryId + "&goods=" + queryConditions.goods+ "&ordernumber=" + queryConditions.ordernumber + "&showZero=" + queryConditions.showZero + "&isSerNum=" + queryConditions.isSerNum) : void a.preventDefault()
