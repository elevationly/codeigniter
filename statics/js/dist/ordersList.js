$(function() {
	...
	function b() {
		...
		j.gridReg("grid", l), l = j.conf.grids.grid.colModel, $("#grid").jqGrid({
			url: "../basedata/inventory/orderslist?action=orderslist&isDelete=2",
			datatype: "json",
			width: a.w,
			height: a.h,
			altRows: !0,
            ...
            gridComplete: function() {
                ...
                $("#grid").find("tr").each(function(){
                    ...
                    var a44 = Number(tdArr.eq(5).text());
                    var a55 = Number(tdArr.eq(6).text());
                    var a66 = Number(tdArr.eq(7).text());
                    var a77 = Number(tdArr.eq(8).text());
                    var a1212 = Number(tdArr.eq(12).text());
                    a4+=a44;
                    a5+=a55;
                    a6+=a66;
                    a7+=a77;
                ...
                $("#grid tbody").append(
                    ...
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'>"+parseFloat(a4.toFixed(3))+"</td>" +
                    ...
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'>"+parseFloat(a12.toFixed(3))+"</td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
	...
	function c() {
		$_matchCon = $("#matchCon"),$_mdescription = $("#mdescription"),$_beizhu = $("#beizhu"),$_cangkuwei = $("#cangkuwei"),$_gongyinshang = $("#gongyinshang"),$_chec = $("#chec"),$_ordernumber = $("#ordernumber"),$_Arrivaltime = $("#Arrivaltime"),$_flagNo = $("#flagNo"), $_matchCon.placeholder(), $("#search").on("click", function(a) {
			a.preventDefault();
			...
			if (Business.verifyRight("INVENTORY_EXPORT")) {
				var b = "按商品编号，商品名称，规格型号等查询" === $_matchCon.val() ? "" : $.trim($_matchCon.val()),
					...
					gongyingshang=$.trim($_gongyinshang.val());
					chec=$_chec.val();
					mdescription=$_mdescription.val();
					beizhu=$_beizhu.val();

					if(flag==0){
						$(this).attr("href", "../basedata/inventory/exporterorder?action=exporterorder&fg=false&isDelete=2&skey=" + b + "&assistId=" + c+"&ordernumber="+ordernumber+"&mdescription="+mdescription+"&Arrivaltime="+Arrivaltime+"&flagNo="+flagNo+"&beizhu="+beizhu+"&chec="+chec+"&cangkuwei="+cangkuwei+'&gongyingshang='+gongyingshang)
					}else{
						$(this).attr("href", "../basedata/inventory/exporterorder?action=exporterorder&fg=true&isDelete=2&skey=" + b + "&assistId=" + c+"&ordernumber="+ordernumber+"&oldnumber="+oldnumber+"&newnumber="+newnumber+"&number="+number+"&Arrivaltime="+Arrivaltime+"&flagNo="+flagNo+"&beizhu="+beizhu+"&chec="+chec+"&cangkuwei="+cangkuwei+'&gongyingshang='+gongyingshang)
					}
		...
		if (a.preventDefault(), Business.verifyRight("INVENTORY_UPDATE")) {
				...
				$.dialog.confirm("您确定要删除该记录吗？", function() {
					$.dialog.prompt("请再次输入密码！", function(a) {
						Public.ajaxGet("../scm/invPu/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								Public.ajaxGet("../scm/invPu/deletestock?action=deletestock", {
									id: b
								}, function(a) {
			...
			if (a.preventDefault(), Business.verifyRight("SA_daohuo")) {
				var b = $("#grid").jqGrid("getGridParam", "selarrrow");
			...
			if (Business.verifyRight("INVENTORY_daoru")) {
				//if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					var c = $(this);
					$.dialog({
						content: "url:../settings/Contractorders",
						data: {
							title: 'excel上传',
							id: "<?php echo $billNo?>",
							callback: function() {
					...
					$.dialog.prompt("请再次输入密码！", function(b) {
						Public.ajaxGet("../scm/invPu/showpwd?action=showpwd", {
							...
							if(200 === b.status){
								...
								Public.ajaxPost("../basedata/inventory/deleteorders?action=deleteorders", {
									id: a
								}, function(b) {
									if (b && 200 == b.status) {
										var c = b.data.id || [];
