			...
			if(SYSTEM.userName=="admin"){
				if(a.checked!=3){
					...
					g = "";
						...
						var c = '<!--<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>-->',
					d = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a id="edit" class="ui-btn">保存</a>',
					e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/lading/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a><a id="zuofei" class="ui-btn ui-btn-sp">作废</a>',
					//e = '<a href="../scm/lading/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
					f = "",
					g = "";
					}
				...
				if(urlParam.flag=="list"){
						...
						billRequiredCheck ? (f = '<a class="ui-btn ui-btn-sp" id="audit">审核</a><a class="ui-btn ui-btn-sp" id="Reject">驳回</a>') : this.$_checkName.parent().hide();
...
//}			
			...
			function k(a, b, c) {
				...
				if ("get" === b) {
					if ("" !== $(".djtypeAuto").getCombo().getValue()) return $(a).val();
					var d = $(a).parents("tr"),
						e = d.data("djtypeInfo") || {};
					return THISPAGE.djtypeCombo.selectByIndex(e.unitId || e.id), e.name || ""
				}
				"set" === b && $("input", a).val(c)
			}
			...
			function dz()
			 {
				 $("#initCombo").append($(".djtypeAuto").val(""))
			 }
			function m(a, b) {
				var c = $(".dateAuto")[0];
				return c
...
//}	
				...
				if (b) {
					...
					$("#grid").jqGrid("showCol", "safeDays"),
					$("#grid").jqGrid("showCol", "validDate"));

					var c = {
						//goodsnumber:b.bumber,
						mdescription:b.name,
						skuName: b.skuName || "",
...
//}
		...
		initCombo: function() {
			...
			this.goodsCombo = Business.billGoodsCombo($(".goodsAuto"), {
				...
				defaultSelected: -1,
				forceSelection: !1
			}), this.djtypeCombo = Business.djtypeCombo($(".djtypeAuto"),{
				//调对应的方法，给其传值
				defaultSelected: -1,
				forceSelection: !1
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/lading/add?action=add", {
				...
				d && c.ajaxPost("../scm/lading/addNew?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/lading/updateInvPu?action=updateInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_CHECK")) {
					...
					d && c.ajaxPost("../scm/lading/checkInvPu?action=checkInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (originalData.id = b.data.id, $("#mark").removeClass(),$("#mark").removeClass("has-noaudit"),$("#mark").addClass("has-audit"), a.$_checkName.html(SYSTEM.realName).parent().show(), $("#edit").hide(), a.disableEdit(), $("#groupBtn").html(a.btn_view + a.btn_reaudit), parent.Public.tips({
							content: "审核成功！"
						})) : parent.Public.tips({
							type: 1,
							content: b.msg
				...
				if (b.preventDefault(), Business.verifyRight("PU_CHECK")) {
					...
					d && c.ajaxPost("../scm/lading/RejectInvPu?action=RejectInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/lading/rsBatchCheckInvPu?action=rsBatchCheckInvPu", {
						id: originalData.id
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_zuofei")) {
				$.dialog.confirm("您确定要作废吗？", function() {
					$.dialog.prompt("请再次输入密码！", function(as) {
						Public.ajaxGet("../scm/lading/showpwd?action=showpwd", {
							userpwd:as,
					...
					d && c.ajaxPost("../scm/lading/zuofeiCheckInvPu?action=zuofeiCheckInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (originalData.id = b.data.id, $("#mark").removeClass(),$("#mark").removeClass("has-audit"),$("#mark").addClass("zuofei"), a.$_checkName.html(SYSTEM.realName).parent().show(), $("#edit").hide(), a.disableEdit(), $("#groupBtn").html(a.btn_view + a.btn_reaudit), parent.Public.tips({
							type: 1,
							content: "作废成功！"
						})) : parent.Public.tips({
							type: 1,
				...
				if (a.preventDefault(), Business.verifyRight("PU_ADD")) {
					...
					parent.tab.overrideSelectedTabItem({
						tabid: c,
						text: b,
						url: "../scm/lading?action=initPur&transType=" + originalData.transType
					})
				}
				...
				//if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					...
					$.dialog({
						...
						close: function() {
							c.ajaxPost("../scm/lading/daoru?action=daoru", {
								id: 2
							}, function(b) {				
							...
							c.ajaxPost("../scm/lading/daorudelete?action=daorudelete", {
								id: 2
							}, function(b) {				
				...
				a.preventDefault(), Business.verifyRight("PU_PRINT") && Public.print({
					title: "物资提货单列表",
					$grid: $("#grid"),
					pdf: "../scm/lading/toPdf?action=toPdf",
					billType: 10101,
					filterConditions: {
						id: originalData.id
					}
				})
			}), $("#grid").on("blur", '.goods', function(a) {
				
				$(".footrow").find(".qtyupd").html("");
			}), $("#grid").on("focus", '.goods', function(a) {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/lading/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(b) { 
					var url=window.location.href;  
					var param=new Object(); 
					var host = window.location.host;
					var pl='http://'+host+'/index.php/scm/lading?action=editPur&id='+param.id+'&flag=list&transType=150501';
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/lading/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(b) {
	...
	if (urlParam.id) {
		...
		if (!hasLoaded) {			
			...
			if (urlParam.turn) Public.ajaxGet("../scm/invPo/queryDetails?action=queryDetails", {
				...
				if (!c.length) return;
				Public.ajaxGet("../basedata/lading?action=list", {
					ids: c.join()
				}, function(a) {
					if (200 === a.status) {
			...
			} else Public.ajaxGet("../scm/lading/update?action=update", {
				id: urlParam.id
			}, function(b) {
