$(function() {
	...
	function b() {
		...
		j.gridReg("grid", l), l = j.conf.grids.grid.colModel, $("#grid").jqGrid({
			url: "../basedata/lading?action=list&isDelete=2",
			datatype: "json",
			width: a.w,
			height: a.h,
			altRows: !0,
			...
			rowNum: 100,
			rowList: [100, 200, 100, 200, 200, 200],
			shrinkToFit: !1,
			forceFit: !0,
			jsonReader: {
				root: "data.rows",
		...
		$_matchCon = $("#matchCon"), $_matchCon.placeholder(), $("#search").on("click", function(a) {
			...
			a.preventDefault(), Business.verifyRight("INVENTORY_ADD") && h.operate("add")
				...
				if (Business.verifyRight("INVENTORY_daoru")) {
					var c = $(this);
					$.dialog({
						content: "url:../settings/Contractmaterial",
						data: {
							title: 'excel上传',
							callback: function() {
			...
			if (Business.verifyRight("INVENTORY_EXPORT")) {
				var b = "按物料编号或物料描述查询" === $_matchCon.val() ? "" : $.trim($_matchCon.val()),
					c = $("#currentCategory").data("id") || "";
				$(this).attr("href", "../basedata/lading/exporter?action=exporter&isDelete=2&skey=" + b + "&assistId=" + c)
			}
				...
				var b = $(this).parent().data("id");
					...
					$.dialog.prompt("请再次输入密码！", function(a) {
						Public.ajaxGet("../scm/lading/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								Public.ajaxGet("../scm/lading/deletegoods?action=deletegoods", {
									id: b
								}, function(a) {
		...
		h = {
			...
			del: function(a) {
				$.dialog.confirm("删除的物料将不能恢复，请确认是否删除？", function() {
					Public.ajaxPost("../basedata/lading/delete?action=delete", {
						id: a
					}, function(b) {
						if (b && 200 == b.status) {
							var c = b.data.id || [];
			...
			setStatus: function(a, b) {
				a && Public.ajaxPost("../basedata/lading/disable?action=disable", {
					invIds: a,
					disable: Number(b)
				}, function(c) {
			...
			setStatuses: function(a, b) {
				if (a && 0 != a.length) {
					var c = $("#grid").jqGrid("getGridParam", "selarrrow"),
						d = c.join();
					Public.ajaxPost("../basedata/lading/disable?action=disable", {
						invIds: d,
						disable: Number(b)
					}, function(c) {
						if (c && 200 == c.status) {
