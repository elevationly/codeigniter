...
var THISPAGE = {
	...
	loadGrid: function() {
		...
		$("#ladingGrids").jqGrid({
			url: "../scm/lading/purListmain?action=purListmain",
			...
			gridview: !0,
			multiselect: !0,
			colNames: ["操作", "单据日期", "单据编号", "项目名", "总数量"/*, "已" + c + "款"*/, "领料人", "制单人", "备注", "订单来源"],
			colModel: [{
				name: "operating",
				width: 60,
				fixed: !0,
			...
			loadComplete: function(a) {
				...
				"150502" == queryConditions.transType && $("#ladingGrids").find(".jqgrow").addClass("red")
			},
			loadError: function() {},
	...
	reloadData: function(a) {
		$("#ladingGrids").jqGrid("setGridParam", {
			url: "../scm/lading/purListmain?action=purListmain",
			datatype: "json",
			postData: a
		}).trigger("reloadGrid")
	...
	addEvent: function() {
		...
		if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
			...
			var b = $(this).parent().data("id"),
				c = $("#ladingGrids").jqGrid("getRowData", b),
				d = 1 == c.disEditable ? "&disEditable=true" : "",
				e = ($("#ladingGrids").jqGrid("getDataIDs"), "物资提货单"),
				f = "purchase-purchase";
			"150502" == queryConditions.transType ? (e = "购货退货单", f = "purchase-purchaseBack", parent.cacheList.purchaseBackId = $("#ladingGrids").jqGrid("getDataIDs")) : parent.cacheList.purchaseId = $("#ladingGrids").jqGrid("getDataIDs"), parent.tab.addTabItem({
				tabid: f,
				text: e,
						...
						Public.ajaxGet("../scm/lading/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								Public.ajaxGet("../scm/lading/delete?action=delete", {
									...
									200 === a.status ? ($("#ladingGrids").jqGrid("delRowData", b), parent.Public.tips({
										content: "删除成功！"
									})) : parent.Public.tips({
										type: 1,
										content: a.msg
		...
		$(".wrapper").on("click", "#btn-batchDel", function(a) {
			...
			var b = $("#ladingGrids").jqGrid("getGridParam", "selarrrow"),
				c = b.join();
			...
			$(".wrapper").on("click", "#audit", function(a) {
				...
				return c ? void Public.ajaxPost("../scm/lading/batchCheckInvPu?action=batchCheckInvPu", {
					...
					if (200 === a.status) {
						for (var c = 0, d = b.length; d > c; c++) $("#ladingGrids").setCell(b[c], "checkName", system.realName), $("#" + b[c]).removeClass("gray");
						parent.Public.tips({
							content: "审核成功！"
						})
					} else parent.Public.tips({
				...
				return c ? void Public.ajaxPost("../scm/lading/rsBatchCheckInvPu?action=rsBatchCheckInvPu", {
					...
					if (200 === a.status) {
						for (var c = 0, d = b.length; d > c; c++) $("#ladingGrids").setCell(b[c], "checkName", "&#160;"), $("#" + b[c]).addClass("gray");
						parent.Public.tips({
							content: "反审核成功！"
						})
					} else parent.Public.tips({
