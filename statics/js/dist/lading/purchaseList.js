...
var THISPAGE = {
	...
	initDom: function() {
		this.$_matchCon = $("#matchCon"),this.$_mname = $("#mname"),this.$_mnumber = $("#mnumber"),this.$_mdescription = $("#mdescription"),this.$_goodsnumber = $("#goodsnumber"), this.$_beginDate = $("#beginDate").val(system.beginDate), this.$_endDate = $("#endDate").val(system.endDate), this.$_matchCon.placeholder(), this.$_beginDate.datepicker(), this.$_endDate.datepicker();
	...
	loadGrid: function() {
		...
		$("#grid").jqGrid({
			url: "../scm/lading?action=list&type=chukudan",
			...
			gridview: !0,
			multiselect: !0,
			colNames: [/*"操作", */"物料编号","物料描述", "数量","单位","钢印号","编号", "项目名"/*, "已" + c + "款"*/, "领料人", "备注", "是否提货", "订单来源"],
			colModel: [/*{
				name: "operating",
				width: 60,
				fixed: !0,
		...
		$("#grid").jqGrid({
			...
			gridview: !0,
			multiselect: !0,
			colNames: ["操作", "单据日期", "单据编号", "项目名"/*, "总数量", "总金额", "已" + c + "款"*/, "领料人", "制单人", "审核人", "备注", "是否提货", "订单来源"],
			colModel: [{
				name: "operating",
				width: 60,
				fixed: !0,
	...
	addEvent: function() {
		...
		if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
			...
			"150502" == queryConditions.transType ? (e = "购货退货单", f = "purchase-purchaseBack", parent.cacheList.purchaseBackId = $("#grid").jqGrid("getDataIDs")) : parent.cacheList.purchaseId = $("#grid").jqGrid("getDataIDs"), parent.tab.addTabItem({
				tabid: f,
				text: e,
				url: "../scm/lading?action=editPur&id=" + b + "&flag=list" + d + "&transType=" + queryConditions.transType
			})
				...
				$.dialog.confirm("您确定要删除该购货记录吗？", function() {
					$.dialog.prompt("请再次输入密码！", function(a) {
						Public.ajaxGet("../scm/lading/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								Public.ajaxGet("../scm/lading/delete?action=delete", {
									id: b
								}, function(a) {
		...
		$(".wrapper").on("click", "#btn-batchDel", function(a) {
			...
			"150502" == queryConditions.transType && (d = "购货退货单"), $.dialog.confirm("您确定要删除选中的" + d + "吗？", function() {
				$.dialog.prompt("请再次输入密码！", function(a) {
						Public.ajaxGet("../scm/lading/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								Public.ajaxPost("../scm/lading/delete?action=delete", {
									id: c
								}, function(a) {
			...
			if(urlParam.Type=="chukudan"){
				var f = "../scm/lading/exportInvPu?action=exportInvPu" + d;
			}else{
				var f = "../scm/lading/exportInvPus?action=exportInvPus" + d;
			}
			...
			$(".wrapper").on("click", "#audit", function(a) {
				...
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void Public.ajaxPost("../scm/lading/batchCheckInvPu?action=batchCheckInvPu", {
					id: c
				}, function(a) {
					if (200 === a.status) {
				...
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void Public.ajaxPost("../scm/lading/rsBatchCheckInvPu?action=rsBatchCheckInvPu", {
					id: c
				}, function(a) {
					if (200 === a.status) {
		...
		function setStatus(a, b) {
			$.dialog.confirm("是否更改提货状态？", function() {
				a && Public.ajaxPost("../scm/lading/get", {
					contactIds: a,
					getStatus: Number(b)
				}, function(c) {
					c && 200 == c.status ? (parent.Public.tips({
						content: "提货状态修改成功！"
					}), $("#grid").jqGrid("setCell", a, "getStatus", b),window.location.reload()) : parent.Public.tips({
						type: 1,
						content: "提货状态修改失败！" + c.msg
					})
				})
			})
		}
		$("#btn-chuku").click(function(a) {
			...
			return b && 0 != b.length ? void setStatus(b, !1) : void parent.Public.tips({
				type: 1,
				content: " 请先选择要更改状态为提货的项目！"
			})
		})
		$("#search").click(function() {
			if(urlParam.Type=="chukudan"){
				queryConditions.getStatus = a.$_getStatus.val();
				queryConditions.matchCon = "请输入单据号或项目名" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.mname = a.$_mname.val(),queryConditions.type = "chukudan", queryConditions.mnumber = a.$_mnumber.val(), queryConditions.mdescription = a.$_mdescription.val(), queryConditions.goodsnumber = a.$_goodsnumber.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
			}else{
				queryConditions.getStatus = a.$_getStatus.val();
				queryConditions.matchCon = "请输入单据号或项目名" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.mname = a.$_mname.val(), queryConditions.mnumber = a.$_mnumber.val(), queryConditions.mdescription = a.$_mdescription.val(), queryConditions.goodsnumber = a.$_goodsnumber.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
			}
			...
			if (a.preventDefault(), Business.verifyRight("PU_ADD")) {
				...
				parent.tab.addTabItem({
					tabid: c,
					text: b,
					url: "../scm/lading?action=initPur&transType=" + queryConditions.transType
				})
			}
