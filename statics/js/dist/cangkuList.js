$(function() {
	...
	function b() {
		...
		j.gridReg("grid", l), l = j.conf.grids.grid.colModel, $("#grid").jqGrid({
			url: "../basedata/inventory/cangkulist?action=cangkulist&isDelete=2",
			datatype: "json",
			width: a.w,
			height: a.h,
			altRows: !0,
            ...
            gridComplete: function() {
                ...
                $("#grid").find("tr").each(function(){
                    ...
                    var a66 = Number(tdArr.eq(6).text());
                    var a77 = Number(tdArr.eq(7).text());
                    var a88 = Number(tdArr.eq(8).text());
                    var a1111 = Number(tdArr.eq(12).text());
                    a5+=a55;
                    a6+=a66;
                    a7+=a77;
                    a8+=a88;
                ...
                $("#grid tbody").append(
                    ...
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'>"+parseFloat(a6.toFixed(3))+"</td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'>"+parseFloat(a7.toFixed(3))+"</td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'>"+parseFloat(a8.toFixed(3))+"</td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;text-align:center;'>"+parseFloat(a11.toFixed(3))+"</td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
                    "<td role='gridcell' style='border-right:1px solid #D6DEE3;border-bottom:1px solid #D6DEE3;'></td>" +
	...
	function c() {
		$_matchCon = $("#matchCon"),$_mdescription = $("#mdescription"),$_ordernumber = $("#ordernumber"),$_beizhu = $("#beizhu"),$_gongyinshang = $("#gongyinshang"),$_cangkuwei = $("#cangkuwei"),$_chec = $("#chec"),$_Arrivaltime = $("#Arrivaltime"),$_flagNo = $("#flagNo"), $_matchCon.placeholder(), $("#search").on("click", function(a) {
			a.preventDefault();
			...
			var b = "按商品编号，商品名称，规格型号等查询" === $_matchCon.val() ? "" : $.trim($_matchCon.val()),
				...
				gongyinshang=$.trim($_gongyinshang.val()),
				flagNo=$.trim($_flagNo.val()),
				cangkuwei=$.trim($_cangkuwei.val()),
				chec=$.trim($_chec.val()),
			...
			$("#grid").jqGrid("setGridParam", {
				...
				postData: {
					...
					beizhu:beizhu,
					gongyinshang:gongyinshang,
					assistId: c
				}
			}).trigger("reloadGrid")
		}), $("#btn-add").on("click", function(a) {
			if (Business.verifyRight("QTSR_xinzen")) {
			var b, c = this;
				c.import_dialog = $.dialog({
					width: 740,
					height: 430,
					title: "新增库存",
					content: "url:../settings/customer_cangku",
					data: {
						curID: a.curID,
						callback: function(b) {
							var d = "上传失败！";
			...
			if (Business.verifyRight("QTSR_daochu")) {
				var b = "按商品编号，商品名称，规格型号等查询" === $_matchCon.val() ? "" : $.trim($_matchCon.val()),
					...
					flagNo=$_flagNo.val();
					cangkuwei=$_cangkuwei.val();
					chec=$_chec.val();
					beizhu=$_beizhu.val();
					if(flag==0){
					$(this).attr("href", "../basedata/inventory/exportcangku?action=exportcangku&fg=false&isDelete=2&skey=" + b + "&assistId=" + c+"&ordernumber="+ordernumber+"&mdescription="+mdescription+"&Arrivaltime="+Arrivaltime+"&flagNo="+flagNo+"&beizhu="+beizhu+"&chec="+chec+"&cangkuwei="+cangkuwei)
					}else{
						$(this).attr("href", "../basedata/inventory/exportcangku?action=exportcangku&fg=true&isDelete=2&skey=" + b + "&assistId=" + c+"&ordernumber="+ordernumber+"&oldnumber="+oldnumber+"&newnumber="+newnumber+"&number="+number+"&Arrivaltime="+Arrivaltime+"&flagNo="+flagNo+"&beizhu="+beizhu+"&chec="+chec+"&cangkuwei="+cangkuwei)				
					}
				
			}
		}), $("#grid").on("click", ".operating .ui-icon-pencil", function(a) {	
		if (a.preventDefault(), Business.verifyRight("QTSR_xiugai")) {
			$(".update_box").show();
			$(".Covering").show();
			var ordersid = $(this).parent().data("id"); 
			Public.ajaxPost("../basedata/inventory/updateorderscangku?action=updateorderscangku", {
						id:ordersid
					}, function(b) {												
						if (b && 200 == b.status) {
							$(".h_us").val(b.id);
			...
			$.dialog.prompt("请再次输入密码！", function(a) {
						Public.ajaxGet("../scm/invPu/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								...
								Public.ajaxPost("../basedata/inventory/updatcangku?action=updatcangku", {
						id:id,
				...
				$.dialog.confirm("您确定要删除该记录吗？", function() {
					$.dialog.prompt("请再次输入密码！", function(a) {
						Public.ajaxGet("../scm/invPu/showpwd?action=showpwd", {
							...
							if(200 === a.status){
								Public.ajaxGet("../scm/invPu/deletecangku?action=deletecangku", {
									id: b
								}, function(a) {
			...
			if (a.preventDefault(), Business.verifyRight("QTSR_chuku")) {
				var b = $("#grid").jqGrid("getGridParam", "selarrrow");
				...
				if (a.preventDefault(), Business.verifyRight("QTSR_daoru")) {
					var c = $(this);
					$.dialog({
		...
		h = {
					...
					$.dialog.prompt("请再次输入密码！", function(b) {
						Public.ajaxGet("../scm/invPu/showpwd?action=showpwd", {
							...
							if(200 === b.status){
								...
								Public.ajaxPost("../basedata/inventory/deletecangku?action=deletecangku", {
									id: a
								}, function(b) {
									if (b && 200 == b.status) {
										var c = b.data.id || [];
			...
			daohuo: function(a) {
				...
				$.dialog({
					...
					height: 510,
					title: '出库',
					content: 'url:../settings/goods_chukubatch?id='+a,
					data: {
						//skuMult: skuMult,
						//skey:_self.skey,
