...
.show li{width:100px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show li:hover{background:#eee;}
</style>
</head>
    ...
    <div class="fl">
      <ul class="ul-inline">
        <li>
		  ...
		  <div style="position:relative;width:111px;float:left;margin-right:5px;">
		  <input type="text" id="mnumber" class="ui-input ui-input-ph" placeholder="请输入物料编号" style="width:100px;">
		  <ul class="show show_ul3" style="position:absolute;top:30px;left:0px;z-index:999;border:1px solid #ddd;display:none;overflow-y:auto;max-height:300px;">
            ...
            <div style="position:relative;width:111px;float:left;margin-right:5px;">
                <input type="text" id="billNo" class="ui-input ui-input-ph" placeholder="请输入单据编号" style="width:100px;">
		...
		-->
          <li><!--<a class="mrb more" id="moreCon">(高级搜索)</a>-->
              <a class="ui-btn" id="search">查询</a>
              <a class="ui-btn ui-btn-refresh" id="refresh" title="刷新"><b></b></a>
          </li>
      </ul>
    </div>
    <div class="fr"><!--<a class="ui-btn ui-btn-sp" id="add">新增</a>--><a class="ui-btn ui-btn-sp" id="guidang">归档</a><a class="ui-btn" id="export" target="_blank" href="javascript:void(0);">导出</a> <a href="#" class="ui-btn d_del" id="btn-batchDel">删除</a><!--<a class="ui-btn" id="print" target="_blank"
...
<script src="<?php echo base_url()?>statics/js/dist/salesLists.js?ver=<?php echo time(); ?>"></script>
...
<script>
    ...
    $(document).bind("click",function(e){
        ...
        if($(e.target).closest("#billNo").length == 0 && $(e.target).closest(".show_ul3").length == 0){
            //点击id为menu之外且id不是不是open，则触发
            $(".show_ul6").css({display:"none"});
        }
    })
    $("#billNo").bind("input propertychange", function () {
        var billNo = $("#billNo").val(); //公司名称
        $.ajax({
            type: "post",
...
$("#mnumber").bind("input propertychange", function () {  
       ...
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querystock7?action=querystock7",  
