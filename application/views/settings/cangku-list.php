...
.show_ul li{width:140px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show_ul li:hover{background:#eee;}
.show_ul2 li{width:140px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show_ul2 li:hover{background:#eee;}
.show_ul3 li{width:140px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show_ul3 li:hover{background:#eee;}
.show_ul4 li{width:140px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show_ul4 li:hover{background:#eee;}
.show_ul5 li{width:140px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
...
<div class="wrapper">
	<div class="mod-search cf">
	    <div class="fl">
	      <ul class="ul-inline">
	        <li>
			  ...
			  <div style="position:relative;width:151px;float:left;margin-right:5px;">
				<input type="text" id="Arrivaltime" class="ui-input ui-input-ph Arrivaltime" placeholder="按申请时间查询" style="width:140px;">
			  ...
			  <div style="position:relative;width:151px;float:left;margin-right:5px;">
				<input type="text" id="cangkuwei" class="ui-input ui-input-ph cangkuwei" placeholder="按仓库位置查询" style="width:140px;">
			  </div>
			  <div style="position:relative;width:151px;float:left;margin-right:5px;">
				<input type="text" id="gongyinshang" class="ui-input ui-input-ph gongyinshang" placeholder="按供应商查询" style="width:140px;">
			  </div>
			  <div style="position:relative;width:101px;float:left;margin-right:5px;">
					<select id="flagNo" style="width:100px;height:30px;">
						...
						<option>未到货</option>
						<option>部分到货</option>
						<option selected>已到货|部分到货</option>
					</select>
			  </div>
			  <div style="position:relative;width:121px;float:left;margin-right:5px;height:30px;line-height:30px;"><input type="checkbox" checked id="chec" value="1"/><span class="chec_span">库存数量大于0</span></div>
			  <style>
				.chec_span{font-size:15px;margin-top:5px;}
			  </style>
			  <!--<div style="position:relative;width:101px;float:left;margin-right:5px;">
	    ...
	    <div class="fr"><a href="#" class="ui-btn ui-btn-sp mrb" id="btn-add">新增</a><!--<a href="#" class="ui-btn mrb" id="btn-print">打印</a>--><a class="ui-btn mrb" id="daohuo">出库</a><!--<a class="ui-btn mrb" id="daoru">导入</a><a class="ui-btn mrb" id="btn-disable">禁用</a><a class="ui-btn mrb" id="btn-enable">启用</a><a href="#" class="ui-btn mrb" id="btn-import">导入</a>--><a href="#" class="ui-btn mrb" id="btn-export">导出</a><a href="#" class="ui-btn a_del" id="btn-batchDel">删除</a></div>
	  </div>
	  <div class="cf">
...
<div class="update_box">
	...
	<form action="../right/upduser?action=upduser" method="post">
		...
		出库数量：<input type="text" name="number" class="number"  disabled="disabled"/><br/><br/>
		库存数量：<input type="text" name="inventoryNew" class="inventoryNew"/>
        已到货数量：<input type="text" name="daohuo" class="daohuo"/><br/><br/>
		...
		项目备注：<input type="text" name="beizhu" class="beizhu"/>
...
<script src="<?php echo base_url()?>statics/js/dist/cangkuList.js?ver=20140430"></script>

<script>	
$("#flagNo").focus(function(){
	$(".show_ul5").css({display:"block"});
})	

$(document).bind("click",function(e){
            //id为menu的是菜单，id为open的是打开菜单的按钮            
            if($(e.target).closest("#flagNo").length == 0 && $(e.target).closest(".show_ul5").length == 0){
            //点击id为menu之外且id不是不是open，则触发
                $(".show_ul5").css({display:"none"});
            }
        }) 
$("#flagNo").bind("input propertychange", function () {  
       var flagNo = $("#flagNo").val(); //公司名称  
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querystock13?action=querystock13",  
           data: { flagNo: flagNo },  
           dataType: "json",  
...
$("#Arrivaltime").bind("input propertychange", function () {  
       ...
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querycangku4?action=querycangku4",  
...
$("#ordernumber").bind("input propertychange", function () {  
       ...
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querycangku3?action=querycangku3",  
...
$("#mdescription").bind("input propertychange", function () {  
       ...
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querycangku2?action=querycangku2",  
...
$("#matchCon").bind("input propertychange", function () {  
       ...
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querycangku?action=querycangku",  
