...
.show_ul li{width:150px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show_ul li:hover{background:#eee;}
.show_ul2 li{width:150px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show_ul2 li:hover{background:#eee;}
.show_ul3 li{width:150px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show_ul3 li:hover{background:#eee;}
.show_ul4 li{width:150px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
.show_ul4 li:hover{background:#eee;}
.show_ul5 li{width:150px;height:25px;line-height:25px;background:#fafdfe;padding-left:10px;}
...
<div class="wrapper">
	<div class="mod-search cf">
	    <div class="fl">
	      <ul class="ul-inline">
	        <li>
				...
				<input type="text" id="matchCon" class="ui-input ui-input-ph matchCon" placeholder="按项目名称查询" style="width:150px;">		
				...
				<div style="position:relative;width:161px;float:left;margin-right:5px;">
					<input type="text" id="ordernumber" class="ui-input ui-input-ph ordernumber" placeholder="按项目定义号查询" style="width:150px;">
			  ...
			  <div style="position:relative;width:161px;float:left;margin-right:5px;">
				<input type="text" id="number" class="ui-input ui-input-ph number" placeholder="按物料号查询" style="width:150px;">
			  ...
			  <div style="position:relative;width:161px;float:left;margin-right:5px;">
				<input type="text" id="mdescription" class="ui-input ui-input-ph mdescription" placeholder="按物料描述查询" style="width:150px;">
				<ul class="show_ul4" style="position:absolute;top:30px;left:0px;z-index:999;border:1px solid #ddd;display:none;overflow-y:auto;max-height:300px;">
					</ul>
			  </div>
	        </li>
	    ...
	    <div class="fr"><!--<a href="#" class="ui-btn ui-btn-sp mrb" id="btn-add">新增</a><a href="#" class="ui-btn mrb" id="btn-print">打印</a><a class="ui-btn mrb" id="daohuo">到货</a>--><a class="ui-btn mrb" id="daoru">导入</a><a href="#" class="ui-btn mrb" id="btn-export">导出</a><a href="#" class="ui-btn a_del" id="btn-batchDel">删除</a><!--<a class="ui-btn mrb" id="btn-disable">禁用</a><a class="ui-btn mrb" id="btn-enable">启用</a><a href="#" class="ui-btn mrb" id="btn-import">导入</a><a href="#" class="ui-btn mrb" id="btn-export">导出</a><a href="#" class="ui-btn a_del" id="btn-batchDel">删除</a>--></div>
	  </div>
	  <div class="cf">
...
<div class="update_box">
	...
	<form action="../right/upduser?action=upduser" method="post">
		...
		项目名称：<input type="text" name="name" class="name"/>
		定&ensp;义&ensp;号：<input type="text" name="ordernumber" class="ordernumbers"/><br/><br/>
		物料编号：<input type="text" name="number" class="numbers"/>
		物料描述：<input type="text" name="mdescription" class="mdescriptions"/><br/><br/>
		出库数量：<input type="text" name="num" class="num"/>
		单&ensp;&ensp;&ensp;&ensp;位：<input type="text" name="mainUnit" class="mainUnit"/><br/><br/>
		出库单价：<input type="text" name="price" class="price"/>
		期初总价：<input type="text" name="amount" class="amount"/><br/><br/>
		施工队伍：<input type="text" name="duiwu" class="duiwu"/>
...
<style>
	...
	.update_box{
		width:700px;height:500px;position:absolute;left:50%;margin-left:-350px;top:50%;margin-top:-250px;z-index:999;background:white;box-shadow:0px 0px 20px rgba(0,0,0,0.3);display:none;
	}
...
<script src="<?php echo base_url()?>statics/js/dist/xiangmuList.js?ver=20140430"></script>
...
$(document).bind("click",function(e){
            ...
            if($(e.target).closest("#ordernumber").length == 0 && $(e.target).closest(".show_ul2").length == 0){
...
$("#ordernumber").bind("input propertychange", function () {  
       ...
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querystock20?action=querystock20",  
...
$(document).bind("click",function(e){
            //id为menu的是菜单，id为open的是打开菜单的按钮            
            if($(e.target).closest("#number").length == 0 && $(e.target).closest(".show_ul3").length == 0){
            //点击id为menu之外且id不是不是open，则触发
                $(".show_ul3").css({display:"none"});
            }
        }) 
$("#number").bind("input propertychange", function () {  
       var number = $("#number").val(); //公司名称  
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querystock19?action=querystock19",  
           data: { number: number },  
           dataType: "json",  
           success: function (data) {
				$(".show_ul3").empty();			   
...
$(document).bind("click",function(e){
            //id为menu的是菜单，id为open的是打开菜单的按钮            
            if($(e.target).closest("#mdescription").length == 0 && $(e.target).closest(".show_ul4").length == 0){
            //点击id为menu之外且id不是不是open，则触发
...
$("#mdescription").bind("input propertychange", function () {  
       ...
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querystock18?action=querystock18",  
...
$("#matchCon").bind("input propertychange", function () {  
       ...
       $.ajax({  
           type: "post",  
           url: "../basedata/inventory/querystock21?action=querystock21",  
