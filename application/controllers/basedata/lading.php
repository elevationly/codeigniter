...
class Lading extends CI_Controller {
	...
	public function index() {
		...
		$list = $this->data_model->get_material($where.' order by a.id desc limit '.$rows*($page-1).','.$rows);
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['spec']          = $row['spec'];
            $v[$arr]['specs']         = $row['spec'];
			$v[$arr]['unitCost']      = $row['unitCost'];
			$v[$arr]['unitName']      = $row['unitName'];
			$v[$arr]['remark']        = $row['remark'];
            $v[$arr]['goods']         = $row['goods'];
            $v[$arr]['jianxing']      = $row['jianxing'];
		}
		$json['status'] = 200;
		$json['msg']    = 'success'; 
		$json['data']['page']      = $page;                                                            
		$json['data']['records']   = $this->data_model->get_material($where,3);    
	...
	public function querystock23(){
		...
		$query=$this->db->query("select distinct(number) from ci_lading_info as a inner join ci_material as b on a.invId=b.id where number like '%$goodsnumber%' ");
	...
	public function querystock16(){
		$mdescription = str_enhtml($this->input->get_post('mdescription',TRUE));
		$query=$this->db->query("select distinct(mdescription) from ci_lading_info where mdescription like '%$mdescription%' and isDelete=0");
		$result=$query->result_array();
		foreach ($result as $arr=>$row) {
			$json[$arr]['mdescription']=$row['mdescription'];
		}
	...
	public function querystock11(){
		$mnumber = str_enhtml($this->input->get_post('mnumber',TRUE));
		$query=$this->db->query("select distinct(billNo) from ci_lading where billNo like '%$mnumber%' and isDelete=0");
		$result=$query->result_array();
		foreach ($result as $arr=>$row) {
	...
	public function querystock10(){
		...
		$query=$this->db->query("select distinct(liname) from ci_lading where liname like '%$mname%' and isDelete=0");
		$result=$query->result_array();
		foreach ($result as $arr=>$row) {
	...
	public function querystock9(){
		$matchCon = str_enhtml($this->input->get_post('matchCon',TRUE));
		$query=$this->db->query("select distinct(a.name),a.number from ci_contact as a inner join ci_lading_info as b on b.buId=a.id where name like '%$matchCon%' or number like '%$matchCon%' ");
		$result=$query->result_array();
		foreach ($result as $arr=>$row) {
	...
	public function querystock8(){
		$mdescription = str_enhtml($this->input->get_post('mdescription',TRUE));
		$query=$this->db->query("select distinct(mdescription) from ci_orders_info  where mdescription like '%$mdescription%' union all select distinct(mdescription) from ci_lading_info   where mdescription like '%$mdescription%'");
		$result=$query->result_array();
		foreach ($result as $arr=>$row) {
			$json[$arr]['mdescription']=$row['mdescription'];
		}
	...
	public function querystock4() {
		$matchCon = str_enhtml($this->input->get_post('matchCon',TRUE));
		$query=$this->db->query("select distinct(number) from ci_material where number like '%$matchCon%' and isDelete=0");
		$result=$query->result_array();
		foreach ($result as $arr=>$row) {
			$json[$arr]['number']=$row['number'];
		}
	...
	public function query() {
		...
		str_alert(200,'success',$this->get_material_info($id)); 
	}
	...
	public function getNextNo() {
		...
		$this->mysql_model->get_count('material',array('isDelete'=>0,'number'=>$skey)) > 0 && str_alert(-1,'商品编号已经存在');
	...
	public function checkBarCode() {
		 ...
		 $this->mysql_model->get_count('material',array('isDelete'=>0,'barCode'=>$barCode)) > 0 && str_alert(-1,'商品条码已经存在');
		 str_alert(200,'success');
	}
	...
	public function add(){
		...
		if ($data) {
			...
			$this->mysql_model->get_count('material',array('isDelete'=>0,'number'=>$data['number'])) > 0 && str_alert(-1,'商品编号重复');
			$this->db->trans_begin();
			$info = array(
                ...
                'unitCost', 'goods', 'jianxing',
			);
			$info = elements($info,$data,NULL);
			$data['id'] = $this->mysql_model->insert('material',$info);
	...
	public function update(){
		...
		if ($data) {
			$data = $this->validform($data);
			$this->mysql_model->get_count('material',array('id !='=>$data['id'],'isDelete'=>0,'number'=>$data['number'])) > 0 && str_alert(-1,'商品编号重复');
			$this->db->trans_begin();
			$info = array(
			    'barCode','baseUnitId','unitName','categoryId','categoryName','propertys',
				'discountRate1','discountRate2','highQty','locationId','pinYin',
			...
			$info = elements($info, $data,NULL);
			$this->mysql_model->update('material',$info,array('id'=>$data['id']));
			if (strlen($data['propertys'])>0) {                          
				$list = (array)json_decode($data['propertys'],true);
				foreach ($list as $arr=>$row) {
					$v[$arr]['invId']         = $data['id'];
            ...
            if ($this->db->trans_status() === FALSE) {
				...
				str_alert(200,'success',$this->get_material_info($data['id']));
			}	 
		}
		str_alert(-1,'修改失败');
	} 
	...
	public function delete(){
		...
		$data = $this->mysql_model->get_results('material','(id in('.$id.')) and (isDelete=0)'); 
		if (count($data) > 0) {
		    $this->mysql_model->get_count('invoice_info','(invId in('.$id.')) and (isDelete=0)')>0 && str_alert(-1,'其中有商品发生业务不可删除');
		    $sql  = $this->mysql_model->update('material',array('isDelete'=>1),'(id in('.$id.'))');   
		    if ($sql) {
	...
	public function exporter() {
		...
		$name = 'material_'.date('YmdHis').'.xls';
		sys_csv($name);
		$this->common_model->logs('导出商品:'.$name);
		$skey = str_enhtml($this->input->get_post('skey',TRUE));
		$where = '(a.isDelete=0)';
		$where .= $skey ? ' and (name like "%'.$skey.'%" or number like "%'.$skey.'%" or spec like "%'.$skey.'%")' : '';
		$data['list']     = $this->data_model->get_material($where.' order by a.id desc');  
        $this->load->view('settings/material-export',$data);
	...
	public function disable(){
		...
		if (strlen($id) > 0) { 
			$sql = $this->mysql_model->update('material',array('disable'=>$disable),'(id in('.$id.'))');
		    if ($sql) {
	...
	//通过ID 获取商品信息
	private function get_material_info($id) {
	    $data = $this->mysql_model->get_rows('material',array('id'=>$id,'isDelete'=>0));
		if (count($data)>0) {
			$data['id']            = $id; 
			$data['count']         = 0;
			$data['name']          = $data['name'];
