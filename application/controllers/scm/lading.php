...
class Lading extends MY_Controller
{
    ...
    public function index()
    {
        ...
        switch ($action) {
            case 'initPur':
                ...
                $data['billNo'] = $this->str_number2();
                $this->load->view('scm/lading/initPur', $data);
                break;
            case 'editPur':
                $this->common_model->checkpurview(233);
                $id = intval($this->input->get_post('id', TRUE));
                $data['billNo'] = $this->mysql_model->get_row('lading', array('id' => $id, 'billType' => 'PUR'), 'billNo');
                $this->load->view('scm/lading/initPur', $data);
                break;
            case 'initPurList':
                // 获取参数
                $checkStatus = $this->input->get('checkStatus', 0);
                $this->common_model->checkpurview(233);
                $this->load->view('scm/lading/initPurList', ['checkStatus' => $checkStatus]);
                break;
            case 'initPurListzuofei':
                $this->common_model->checkpurview(233);
                $this->load->view('scm/lading/initPurListzuofei');
                break;
            default:
                $this->common_model->checkpurview(233);
                $this->purList();
    ...
    public function purList()
    {
        ...
        $checkStatus = intval($this->input->get_post('checkStatus', TRUE));
        ...
        $getStatus = $this->input->get_post('getStatus', TRUE);
        ...
        if ($type == "chukudan") {
            $where = 'a.isDelete=0 and f.checked';
            $where .= $checkStatus == 3 ? '=3' : '!=3';
            ...
            $where .= in_array($getStatus, ['0', '1'], true) ? ' and a.getStatus = ' . $getStatus : '';
        ...
        if ($type == "chukudan") {
            //echo $type."=============";
            $lists = $this->data_model->get_lading_info($where . ' order by ' . $order . ' limit ' . $rows * ($page - 1) . ',' . $rows);
            ...
            $list = $this->data_model->get_lading($where . ' order by ' . $order . ' limit ' . $rows * ($page - 1) . ',' . $rows);
        ...
        if ($type == "chukudan") {
            $json['data']['records'] = $this->data_model->get_lading_info($where, 3);
            $json['data']['total'] = ceil($json['data']['records'] / $rows);
        } else {
            $json['data']['records'] = $this->data_model->get_lading($where, 3);
            $json['data']['total'] = ceil($json['data']['records'] / $rows);
        }
    ...
    public function delivery()
    {
        ...
        // 判断单据是否为空，是否为提货单
        if (empty($billNo) || strpos($billNo, 'THD') !== 0) {
            // 返回错误信息
            str_alert(-1, '未成功识别提货单单号');
        }
        // 获取单据图片
        $imageBase64 = $this->input->get_post('image', TRUE);
        // 处理图片
        $imageBase64 = base64_decode($imageBase64);
        // 构建单据图片保存地址
        $imageSavePath = sprintf('%s%s/%s', BASEPATH, '../statics/receipt/lading', $billNo.'.jpg');
        // 保存单据图片
        file_put_contents($imageSavePath, $imageBase64);
        // TODO 图片分析

        // 更新单据状态
        $this->db->where('billNo',$billNo)->update('ci_lading',['getStatus'=>1]);
        // 保存日志
        $this->common_model->logs('客户提货单已提货:billNo:'.$billNo);
        // 返回成功
    ...
    public function purListmain()
    {
        ...
        if ($type == "chukudan") {
            //echo $type."=============";
            $lists = $this->data_model->get_lading_info($where);
            //print_r($lists);
            //exit();
            // 计算总记录数
            ...
            $list = $this->data_model->get_lading($where . ' order by ' . $order . ' limit ' . $offset . ',' . $rows);
            foreach ($list as $arr => $row) {
                $v[$arr]['hxStateCode'] = intval($row['hxStateCode']);
    ...
    {
        $this->common_model->checkpurview(237);
        ...
        ///$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : '';
        //$where .= $this->common_model->get_admin_purview();
        //$data['list'] = $this->data_model->get_lading($where.' order by '.$order);
        //$list = $this->data_model->get_lading($where.' order by '.$order);
        //foreach ($list as $arr=>$row) {

        $lists = $this->data_model->get_lading_info($where . ' order by a.id');
        //print_r($this->db->last_query());exit;
        foreach ($lists as $arr => $row) {
            $v[$arr]['invNumber'] = $row['invNumber'];
            $v[$arr]['mdescription'] = $row['mdescription'];
        ...
        //exit();
        $data['list'] = $v;
        $this->load->view('scm/lading/exportInvPu', $data);
    }
    ...
    public function exportInvPus()
    {
        ...
        //$list = $this->data_model->get_lading($where.' order by '.$order);
        //foreach ($list as $arr=>$row) {
        $lists = $this->data_model->get_lading($where);
        foreach ($lists as $arr => $row) {
            $v[$arr]['hxStateCode'] = intval($row['hxStateCode']);
        ...
        //exit();
        $data['list'] = $v;
        $this->load->view('scm/lading/exportInvPus', $data);
    }
    ...
    public function add()
    {
        ...
        if (strlen($data) > 0) {
            ...
            $this->db->trans_begin();
            $iid = $this->mysql_model->insert('lading', $info);
            $this->lading_info($iid, $data);
    ...
    {
        $this->common_model->checkpurview(235);
        $data = $this->input->post('postData', TRUE);
        if (strlen($data) > 0) {
            $data = $this->validform((array)json_decode($data, true));
            $info = elements(array(
            ...
            $this->db->trans_begin();
            $this->mysql_model->update('lading', $info, array('id' => $data['id']));
            $this->lading_info($data['id'], $data);
    ...
    public function update()
    {
        ...
        $id = intval($this->input->get_post('id', TRUE));
        $data = $this->data_model->get_lading('a.isDelete=0 and a.id=' . $id . ' and a.billType="PUR"', 1);
        //print_r($data);
        //exit;
        if (count($data) > 0) {
            $info['status'] = 200;
            ...
            $list = $this->data_model->get_lading_info('a.isDelete=0 and a.iid=' . $id . ' order by a.id');
    ...
    {
        $this->common_model->checkpurview(238);
        ...
        if (count($data) > 0) {
            ...
            $dataCount = count($postData['entries']);
            $data['num'] = min(max(8, $dataCount), 25);
            ...
            // 条形码
            $this->common_model->barcode($data['billNo']);
            // 开始处理
            ob_start();
            $this->load->view('scm/lading/toPdf', $data);
            ...
            try {
                ...
                $html2pdf->Output('lading_' . date('YmdHis') . '.pdf');
    ...
    {
        $this->common_model->checkpurview(236);
        $id = str_enhtml($this->input->get_post('id', TRUE));
        $data = $this->mysql_model->get_results('lading', '(isDelete=0) and (id in(' . $id . ')) and billType="PUR"');
        if (count($data) > 0) {
            ...
            $this->db->trans_begin();
            $this->mysql_model->update('lading', array('isDelete' => 1), '(id in(' . $id . '))');
            $this->mysql_model->update('lading_info', array('isDelete' => 1), '(iid in(' . $id . '))');
    ...
    public function deletegoods()
    {
        ...
        $id = str_enhtml($this->input->get_post('id', TRUE));
        $this->db->trans_begin();
        $this->mysql_model->update('material', array('isDelete' => 1), '(id in(' . $id . '))');
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            str_alert(-1, '删除失败');
        } else {
    ...
    public function delete1()
    {
        $this->common_model->checkpurview(236);
        $id = intval($this->input->get('id', TRUE));
        $data = $this->mysql_model->get_rows('lading', array('id' => $id, 'billType' => 'PUR'));
        if (count($data) > 0) {
            //$data['checked'] >0 && str_alert(-1,'已审核的不可删除');
            $this->db->trans_begin();
            $this->mysql_model->update('lading', array('isDelete' => 1), array('id' => $id));
            $this->mysql_model->update('lading_info', array('isDelete' => 1), array('iid' => $id));
            if ($data['accId'] > 0) {
    ...
    {
        $this->common_model->checkpurview(239);
        $data = $this->input->post('postData', TRUE);
    ...
    public function batchCheckInvPu()
    {
        ...
        $data = $this->mysql_model->get_results('lading', '(id in(' . $id . ')) and billType="PUR" and isDelete=0');
        if (count($data) > 0) {
            foreach ($data as $arr => $row) {
            ...
            $sql = $this->mysql_model->update('lading', array('checked' => 1, 'checkName' => $this->jxcsys['name']), '(id in(' . $id . '))');
            if ($sql) {
    ...
    {
        $this->common_model->checkpurview(240);
        $id = str_enhtml($this->input->post('id', TRUE));
        $data = $this->mysql_model->get_results('lading', '(id in(' . $id . ')) and billType="PUR" and isDelete=0');
        if (count($data) > 0) {
            foreach ($data as $arr => $row) {
            ...
            $sql = $this->mysql_model->update('lading', array('checked' => 0, 'checkName' => ''), '(id in(' . $id . '))');
            if ($sql) {
    ...
    private function validform($data)
    {
        ...
        if ($data['id'] > 0) {
            $invoice = $this->mysql_model->get_rows('lading', array('id' => $data['id'], 'billType' => 'PUR', 'isDelete' => 0));
...
//		}
    ...
    private function lading_info($iid, $data)
    {
        foreach ($data['entries'] as $arr => $row) {
            $v[$arr]['iid'] = $iid;
            $v[$arr]['uid'] = $data['uid'];
        ...
        if (isset($v)) {
            if ($data['id'] > 0) {
                $this->mysql_model->delete('lading_info', array('iid' => $iid));
            }
            $this->mysql_model->insert('lading_info', $v);
        }
    }
    ...
    public function get()
    {
        $status = intval($this->input->post('getStatus',TRUE));
        ...
        if (is_array($id)) {
            //修改orders_info里仓库状态
            $this->db->where_in('id',$id)->update('ci_lading',['getStatus'=>1]);

            $this->common_model->logs('客户提货单'.$status==1?'已提货':'未提货'.':ID:'.implode(',', $id).'');
            str_alert(200,'success');
        }
        str_alert(-1,'操作失败');
    }
	...
	public function getImagesById() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['status']       = 1;
			$v[$arr]['name']         = $row['name'];
			$v[$arr]['url']          = site_url().'/scm/lading/getImage?action=getImage&pid='.$row['id'];
			$v[$arr]['thumbnailUrl'] = site_url().'/scm/lading/getImage?action=getImage&pid='.$row['id'];
			$v[$arr]['deleteUrl']    = '';
			$v[$arr]['deleteType']   = '';
		}
		$json['status'] = 200;
	...
	public function postexcel($filename)
	{
	    ...
	    $this->importExcelMaterial($jddir.'\data\upfile\excel\\'.$filename);
	}
	...
	public function uploadexcelgoods()
    {
        ...
        if($type==".xlsx"||$type==".xls") {
            ...
            if (in_array(PHP_OS, ['Linux'])) {
                ...
                $this->importExcelMaterial('/tmp/'.$name);
            } else {
                $name=explode('.',$_FILES["file"]["name"]);
                $date=date('Ymdhis');
                $newPath=$date.'.'.$name[1];
	...
	public function uploadImages() {
		...
		$files[0]['url']          = site_url().'/scm/lading/getImage?action=getImage&pid='.$newid;
		$files[0]['thumbnailUrl'] = site_url().'/scm/lading/getImage?action=getImage&pid='.$newid;
		$files[0]['deleteUrl']    = '';
		$files[0]['deleteType']   = '';
		$json['status'] = 200;
		$json['msg']    = 'success';
