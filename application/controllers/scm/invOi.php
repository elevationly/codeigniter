...
class InvOi extends CI_Controller {
	...
	public function queryToPD() {
		...
		$where = '(a.isDelete=0)';
		$where .= $goods ? ' and a.goodsnumber="'.$goods.'"' : ''; 
		$where .= $ordernumber ? ' and a.ordernumber="'.$ordernumber.'"' : '';  
		$list = $this->data_model->get_stock($where.' GROUP BY a.id'.$having.' limit '.$rows*($page-1).','.$rows);  
		foreach ($list as $arr=>$row) {
		...
		$json['msg']    = 'success'; 
		$json['data']['page']         = $page;
		//$json['data']['records']      = $this->data_model->get_stock($where.' GROUP BY a.invId,a.locationId'.$having,3);    
		//$json['data']['total']        = ceil($json['data']['records']/$rows);                           
		$json['data']['rows']         = isset($v) ? $v : array();
		die(json_encode($json));
	}
	...
	public function queryToPD() {
		...
		$list = $this->data_model->get_stock($where.' GROUP BY a.invId,a.locationId '.$having.' limit '.$rows*($page-1).','.$rows);  
		print_r($list);
		exit();
		foreach ($list as $arr=>$row) {
	...
	public function exportToPD() {
		...
		$where = '(a.isDelete=0)';
		$where .= $goods ? ' and a.goodsnumber="'.$goods.'"' : ''; 
		$where .= $ordernumber!="undefined" ? ' and a.ordernumber="'.$ordernumber.'"' : '';  
		//$data['list'] = $this->data_model->get_stock($where);
		$sql="select * from ci_stock as a where ".$where;
