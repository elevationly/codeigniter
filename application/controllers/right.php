...
class Right extends CI_Controller {
	...
	public function upduser(){
		$h_us = str_enhtml($this->input->get_post('h_us',TRUE));
		$username = str_enhtml($this->input->get_post('username',TRUE));
		$uso = md5(str_enhtml($this->input->get_post('uso',TRUE)));
		$updp = md5(str_enhtml($this->input->get_post('updp',TRUE)));	
		$truename = str_enhtml($this->input->get_post('truename',TRUE));	
		$data=array(
			'username'=>$username,
			'userpwd'=>$updp,
			'name'=>$truename,
		);
		$bool=$this->db->update('admin',$data,array('username'=>$h_us,'userpwd'=>$uso));//将数据库.user表里id=3的用户密码给为12345
	...
	public function showuser(){
		$userName = str_enhtml($this->input->get_post('user',TRUE));
		$userpwd = $this->input->get_post('pwd',TRUE);
	...
	public function deluser(){
		...
		$bool = $this->db->query("delete from ci_admin where uid='$id'");
		$json['status']              = 200;
			$json['msg']                 = 'success'; 
			die(json_encode($json));
	}
	...
	public function adduser() {
		...
		if (is_array($data)&&count($data)>0) {
			...
			if ($sql) {
				...
				$query=$this->db->query("select id from ci_contact");
				...
				$username=$data['userNumber'];
				$newstr = substr($arrs,0,strlen($arrs)-1);
				$query=$this->db->query("update ci_admin set righttype2='$newstr' where username='$username'");
