...
if (!function_exists('skin_url')) {
	function skin_url() {
		// 统一使用 app2_beta，使 Chrome 与其它浏览器共用同一套 jQuery/JS，项目名/领料人「...」等可正常触发
...
if (!function_exists('str_number')) {
	function str_number($str='') {
		return $str.rand(0,9).rand(0,9).rand(0,9).rand(0,9);
	}
}
...
if (!function_exists('mb_str_split')) {
    function mb_str_split($str = '', $length = 60) {
        // 判断字符串是否小于长度
        ...
        // 截取
        $tempStr = mb_substr($str, 0, $length, 'UTF-8');
        // 拼接后续
        $tempStr .= '<br/>'.mb_str_split(mb_substr($str, $length, null, 'UTF-8'));
        // 返回
        return $tempStr;
    }
}
