...
//库存统计
	public function get_stock($where='',$type=2) {
	    $sql = 'select * from '.$this->db->dbprefix('stock').' as a where '.$where;
		return $this->mysql_model->query($sql,$type);
	}
	public function get_stocks($where='',$type=2) {
	    $sql = 'select * from '.$this->db->dbprefix('stock').' as a where '.$where;
		return $this->mysql_model->query($sql,$type);
	}
	public function get_cangku($where='',$type=2) {
	    $sql = 'select * from '.$this->db->dbprefix('cangku').' as a where '.$where;
	    // var_dump($sql);die();
		return $this->mysql_model->query($sql,$type);
	}
	public function get_cangkuhuizong($where='',$type=2) {
	    $sql = 'select * from '.$this->db->dbprefix('cangku_huizong').' as a where '.$where;
		return $this->mysql_model->query($sql,$type);
	}

	public function get_xiangmu($where='',$type=2) {
	    $sql = 'select * from '.$this->db->dbprefix('xiangmuku').' as a where '.$where;
		return $this->mysql_model->query($sql,$type);
	}
    ...
    //获取单据列表
    public function get_lading($where='',$type=2) {
        $sql = 'select 
		            a.*,
					b.name as contactName,b.number as contactNo,   
					c.number as salesNo ,c.name as salesName, 
					d.number as accountNumber ,d.name as accountName,
					(a.rpAmount + ifnull(e.nowCheck,0)) as hasCheck
				from '.$this->db->dbprefix('lading').' as a 
					left join 
						(select id,number, name from '.$this->db->dbprefix('contact').' where isDelete=0) as b
					on a.buId=b.id 
					left join 
	...
	//获取单据列表
//	public function get_orders($where='',$type=2) {
//	    $sql = 'select
//		            a.*,
//					b.name as contactName,b.number as contactNo,
//					c.number as salesNo ,c.name as salesName,
...
//	}
    ...
    public function get_orders($where='',$type=2) {
        ...
        $sql = 'select '.$select.' 
				from '.$this->db->dbprefix('orders').' as a 
					left join '.$this->db->dbprefix('contact').' as b on a.buId=b.id and b.isDelete=0 
					left join '.$this->db->dbprefix('staff').' as c on a.salesId=c.id and c.isDelete=0 
					left join '.$this->db->dbprefix('account').' as d on a.accId=d.id and d.isDelete=0 
				where '.$where;
	    if($type == 3){
	...
	//获取单据列表明细
	public function get_invoice_info($where='',$type=2,$checked = '') {
        $check_sql = $checked == '!3' ? ' and checked!=3' : '' ;
	    $sql = 'select 
		            a.*, 
					b.name as invName, b.number as invNumber, b.spec as invSpec, 
					b.unitName as mainUnit,b.pinYin,b.purPrice,b.quantity,b.salePrice,b.unitId,
					c.number as contactNo, c.name as contactName,
					d.name as locationName ,d.locationNo ,
					e.number as salesNo ,e.name as salesName,f.checked as checked
				from '.$this->db->dbprefix('invoice_info').' as a 
					left join 
						(select id,name,number,spec,unitName,unitId,pinYin,purPrice,quantity,salePrice from '.$this->db->dbprefix('goods').' where isDelete=0) as b 
					on a.invId=b.id 
					...
					on a.salesId=e.id
					left join 
						(select id,checked from '.$this->db->dbprefix('invoice').' where isDelete=0 '.$check_sql.') as f
					on f.id=a.iid
				where '.$where;
    ...
    //获取单据列表明细
    public function get_lading_info($where='',$type=2,$checked = '') {
        $check_sql = $checked == '!3' ? ' and checked!=3' : '' ;
        $sql = 'select 
		            a.*, 
					b.name as invName, b.number as invNumber, b.spec as invSpec, 
					...
					d.name as locationName ,d.locationNo ,
					e.number as salesNo ,e.name as salesName,f.checked as checked
				from '.$this->db->dbprefix('lading_info').' as a 
					left join 
						(select id,name,number,spec,unitName,unitId,pinYin,purPrice,quantity,salePrice from '.$this->db->dbprefix('material').' where isDelete=0) as b 
					on a.invId=b.id 
					left join 
						(select id,number, name from '.$this->db->dbprefix('contact').' where isDelete=0) as c
					on a.buId=c.id 	
					...
					on a.salesId=e.id
					left join 
						(select id,checked from '.$this->db->dbprefix('lading').' where isDelete=0 '.$check_sql.') as f
					on f.id=a.iid
				where '.$where;
	...
	//获取单据列表明细
	public function get_invoice_infos($where='',$type=2) {
			/*
		$contactwhere="";
		$goodwhere="";
		$llrwhere="";
		if(!empty($where))
		{
			$contactwhere='where isDelete=0 '.$where;
		}
		if(!empty($cwhere))
		{
			 $goodwhere='where isDelete=0 '.$cwhere;
		}

		if(!empty($llwhere))
		{
			$llrwhere='where '.$llwhere;
		}
	    ...
	    /* $sql = 'select
				...
				from '.$this->db->dbprefix('invoice_info').' as a
					left join
						(select id,name,number,spec,unitName,unitId,pinYin,purPrice,quantity,salePrice from '.$this->db->dbprefix('goods').' '.$goodwhere.' ) as b
					on a.invId=b.id
					left join
						(select id,number, name from '.$this->db->dbprefix('contact').' '.$contactwhere.') as c
					on a.buId=c.id
				 '.$llrwhere;
*/
    ...
    $sql = 'select 
					...
					b.name as invName, b.number as invNumber, b.spec as invSpec, 
					b.unitName as mainUnit,b.pinYin,b.purPrice,b.quantity,b.salePrice,b.unitId,
					c.number as contactNo, c.name as contactName,d.checked as checked
					 
				from '.$this->db->dbprefix('invoice_info').' as a 
					left join  
						 '.$this->db->dbprefix('goods').' as b 
					on a.invId=b.id 
					left join  
						'.$this->db->dbprefix('contact').' as c
					on a.buId=c.id 
					left join 
						(select id,checked from '.$this->db->dbprefix('invoice').' where isDelete=0 and checked!=3) as d
					on a.iid=d.id
				   where '.$where;


/*
select   a.*, b.name as invName, b.number as invNumber, b.spec as invSpec, b.unitName as mainUnit,b.pinYin,b.purPrice,b.quantity,b.salePrice,b.unitId,c.number as contactNo, c.name as contactName from ci_invoice_info as a
					left join ci_goods as b
					on a.invId=b.id
					left join ci_contact as c
					on a.buId=c.id
				 where a.isDelete=0 and (c.number like '%10100%'or c.name like '%10100%' )order by  a.id desc limit 0,100
	...
	//获取单据列表明细
	public function get_orders_info($where='',$type=2,$checked = 0) {
	    $check_sql = $checked == 3 ? 'checked=3' : 'checked!=3' ;
        $sql = 'select a.*,c.number as contactNo,
                       c.name as contactName,
                       d.checked as checked, 
                       c.cCategoryName as customerType, 
                       c.disable as disable,
                       c.design as design,
                       c.apply as apply,
                       c.check as cCheck
                from '.$this->db->dbprefix('orders_info').' as a 
				left join 
						(select * from '.$this->db->dbprefix('contact').' where isDelete=0) as c
					on a.buId=c.id
				left join 
						(select id,checked from '.$this->db->dbprefix('orders').' where isDelete=0 and '.$check_sql.') as d
					on a.iid=d.id
				where '.$where;
		return $this->mysql_model->query($sql,$type);
	}

	public function get_stock_yj($wheres='',$where='',$type=2) {
	    $sql = 'select a.goodsnumber,a.mdescription,sum(a.inventoryNew) as numyj,a.mainUnit,b.yjnum,b.id from ci_stock as a left join ci_stock_yj as b on a.id=b.sid where '.$wheres.' group by a.goodsnumber having a.goodsnumber in (select number from ci_stock_yj) '.$where;
		return $this->mysql_model->query($sql,$type);
	}
	//获取单据列表明细
	/*
	public function get_orders_info($where='',$type=2) {
	    $sql = 'select
		            a.*,
					b.name as invName, b.number as invNumber, b.spec as invSpec,
					b.unitName as mainUnit,b.pinYin,b.purPrice,b.quantity,b.salePrice,b.unitId,
	...
	public function get_goods($where='',$type=2) {
	    $sql = 'select a.* from '.$this->db->dbprefix('goods').' as a where '.$where;
		return $this->mysql_model->query($sql,$type);
	}

    //获取商品明细
    public function get_material($where='',$type=2) {
        $sql = 'select a.* from '.$this->db->dbprefix('material').' as a where '.$where;
        return $this->mysql_model->query($sql,$type);
    }
	/*
	public function get_goods($where='',$type=2) {
...
//获取商品明细
	public function get_goodsorder($where='',$type=2) {
	    $sql = 'select 
					a.*,b.iniqty,b.iniunitCost,b.iniamount,b.totalqty
				from '.$this->db->dbprefix('stock').' as a 
				left join 
					(select 
						invId,
						sum(qty) as totalqty, 
						...
						sum(case when billType="INI" then price else 0 end) as iniunitCost,
						sum(case when billType="INI" then amount else 0 end) as iniamount
					from '.$this->db->dbprefix('orders_info').' 
					where isDelete=0 group by invId) as b 
				on a.id=b.invId  where '.$where;
		return $this->mysql_model->query($sql,$type);
	}
