...
class Common_model extends CI_Model{
    ...
    public function stock_logs($info,$id,$ordernumber='',$goodsnumber='')
    {
        $data['userId']     =  $this->jxcsys['uid'];
        $data['name']       =  $this->jxcsys['name'];
        $data['ip']         =  $this->input->ip_address();
        ...
        $data['adddate']    =  date('Y-m-d H');
        $data['modifyTime'] =  date('Y-m-d H:i:s');
        $this->mysql_model->insert('stock_log',$data);
    }

    public function cangku_logs($info,$id,$ordernumber='',$goodsnumber='')
    {
        $data['userId']     =  $this->jxcsys['uid'];
        $data['name']       =  $this->jxcsys['name'];
        $data['ip']         =  $this->input->ip_address();
        ...
        $data['adddate']    =  date('Y-m-d H');
        $data['modifyTime'] =  date('Y-m-d H:i:s');
        $this->mysql_model->insert('cangku_log',$data);
    }
    ...
    {
        include (FCPATH.'statics/barcode/class/BCGColor.php');
        include (FCPATH.'statics/barcode/class/BCGDrawing.php');
        include (FCPATH.'statics/barcode/class/BCGcode128.barcode.php');
        $colorFront = new BCGColor(0, 0, 0);
        $colorBack = new BCGColor(255, 255, 255);

        // Barcode Part
        $font = new BCGFontFile(FCPATH.'statics/barcode/font/Arial.ttf', 10);
        $code = new BCGcode128();
        $code->setScale(2);
        $code->setFont($font);
        $code->setColor($colorFront, $colorBack);
        $code->parse($text);
        $code->setThickness(15); // 条形码的厚度

        // Drawing Part
        $drawing = new BCGDrawing(FCPATH."statics/barcode.png", $colorBack);
